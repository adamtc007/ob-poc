...
('isda', 'isda.margin_call',
 ...
 '{
   ":exposure-amount": {"business_meaning": "Current mark-to-market exposure that determines collateral requirement", "validation": "Must be accurate valuation as of valuation date"},
   ":call-amount": {"business_meaning": "Amount of additional collateral being requested", "validation": "Must exceed minimum transfer amount per CSA"},
   ":deadline": {"business_meaning": "Deadline for counterparty to post collateral, affects default risk", "validation": "Must allow reasonable time per CSA terms"}
 }',
 'collateral_management',
 ARRAY['Creates counterparty credit exposure', 'May trigger default if not met', 'Affects regulatory capital calculations'],
 'high'),

-- isda.close_out semantic metadata
('isda', 'isda.close_out',
 'Performs early termination and close-out netting calculation following termination event, determining final settlement amount between parties under ISDA Master Agreement',
 'terminate',
 'Calculate final settlement obligations when ISDA Master Agreement is terminated early due to default or other termination events, enabling clean break between parties',
 ARRAY['Terminates all outstanding trades', 'Creates final settlement obligation', 'Ends ongoing credit exposure'],
 ARRAY['Valid termination event must have occurred', 'Close-out valuation methodology must be specified', 'Market data must be available'],
 ARRAY['All trades are terminated', 'Final settlement amount is determined', 'Legal relationship is concluded'],
 'Use this verb when early termination occurs due to default or other termination events. The close-out calculation determines the final amount owed between parties. Ensure valuation methodology follows Master Agreement terms.',
 '{
   ":valuation-method": {"business_meaning": "Method for determining terminated trade values, affects final settlement amount", "validation": "Must follow Master Agreement specified methodology"},
   ":closeout-amount": {"business_meaning": "Net amount owed by one party to the other after netting all terminated trades", "validation": "Must be calculated per ISDA close-out methodology"},
   ":payment-date": {"business_meaning": "Date when final settlement payment is due", "validation": "Must comply with Master Agreement payment terms"}
 }',
 'termination',
 ARRAY['Finalizes legal obligations', 'May trigger regulatory reporting', 'Affects credit loss calculations'],
 'high');
...
-- Define relationships between ISDA verbs and other domain verbs
...
-- Master Agreement establishment enables CSA
('isda', 'isda.establish_master', 'isda', 'isda.establish_csa', 'enables', 1.0,
 'CSA requires existing Master Agreement as legal foundation', 'before'),

-- Master Agreement enables trade execution
('isda', 'isda.establish_master', 'isda', 'isda.execute_trade', 'enables', 1.0,
 'All derivative trades must be executed under existing Master Agreement', 'before'),

-- CSA enables margin calls
('isda', 'isda.establish_csa', 'isda', 'isda.margin_call', 'enables', 1.0,
 'Margin calls require CSA defining collateral terms', 'before'),

-- Trade execution triggers portfolio valuation
('isda', 'isda.execute_trade', 'isda', 'isda.value_portfolio', 'enables', 0.9,
 'New trades require portfolio revaluation for risk management', 'after'),

-- Portfolio valuation enables margin calls
('isda', 'isda.value_portfolio', 'isda', 'isda.margin_call', 'enables', 0.9,
 'Margin calls based on current portfolio valuation', 'after'),

-- Margin calls enable collateral posting
('isda', 'isda.margin_call', 'isda', 'isda.post_collateral', 'enables', 1.0,
 'Collateral posting responds to margin calls', 'after'),

-- Termination events enable close-out
('isda', 'isda.declare_termination_event', 'isda', 'isda.close_out', 'enables', 1.0,
 'Close-out follows termination event declaration', 'after'),

-- Document cataloging supports ISDA workflows
('document', 'document.catalog', 'isda', 'isda.establish_master', 'enables', 0.8,
 'Master Agreement documents should be cataloged', 'before'),

-- Document verification supports ISDA establishment
('document', 'document.verify', 'isda', 'isda.establish_master', 'enables', 0.7,
 'Verified documents increase confidence in ISDA setup', 'before'),

-- ISDA trade execution triggers document usage
('isda', 'isda.execute_trade', 'document', 'document.use', 'enables', 0.8,
 'Trade confirmations should be tracked as document usage', 'parallel'),

-- Entity and edge creation supports ISDA counterparty setup
('graph', 'entity', 'isda', 'isda.establish_master', 'enables', 0.9,
 'Counterparty entities must exist before Master Agreement', 'before'),

('graph', 'edge', 'isda', 'isda.establish_master', 'enables', 0.7,
 'Corporate relationships may inform ISDA setup', 'before');

-- ISDA domain is now fully configured and ready for V3 workflows
