...
> **Current downstream state (2026-03-11):** `ob-poc` has cut over to a three-step utterance pipeline for the SemTaxonomy path: `EntityScope -> EntityState -> SelectedVerb`. Semantic OS remains the governance/contract layer and transition substrate, while `ob-poc` composes proposals from grounded live state plus SemOS-valid verbs. The old SemTaxonomy composition path is removed from normal proposal selection. The current StateGraph substrate (graph model, loader, walker, `discovery.graph-walk`, graph-backed Step 2) is implemented, and the safe Phase 1 metadata cleanup is complete. Unsafe graph/corpus rewrites are intentionally parked pending the authoritative external correction table for unresolved verb families such as `struct.*` and `screening.full`. Current live harness metrics for the new path are `14/176` exact top-verb hits (`7.95%`), `125/176` grounded/stateful responses (`71.02%`), and `84/176` business proposals, which confirms the simplified boundary is in place and that the main remaining limiter is metadata integrity rather than routing architecture.
>
> **Downstream Sem OS governance state (2026-03-18):** `ob-poc` has now completed the envelope/file cutover on the Sem OS governance path: `SemOsContextEnvelope` is the live downstream carrier for `resolve_context()` results, the old in-repo `sem_reg::context_resolution` module has been removed, and REPL/chat execution paths are fail-closed for utterance discovery when Sem OS is unavailable. Discovery-stage responses also carry bootstrap fields (`resolution_stage`, `discovery_surface`) so empty or lightly grounded sessions can render domain/family/constellation guidance without widening back to raw global verb search.
>
> **Downstream reconciliation remediation state (2026-03-19):** the post-cutover reconciliation gaps identified in `docs/todo/semos-reconciliation-findings-2026-03-19.md` have been closed in the active `ob-poc` integration layer. Direct verb-search paths now thread `entity_kind`; canonical kinds were expanded (`fund`, `document`, `contract`, `trading-profile`, `deal`, `cbu`); runtime registry and Sem OS both honor `subject_kinds` with explicit entity-kind prune reporting and confidence-aware widening; discovery bootstrap questions are actionable in chat; authored constellation families for trading/billing/deal/contract now participate in family selection and verb-reference validation; macro output is re-validated after expansion; fail-closed safe-harbor verbs are audited against `harm_class` at startup; and parked runbook reasons are exposed through the chat/session response surface.
>
> **Downstream hygiene state (2026-03-19):** after the reconciliation landing, the broader Rust workspace was brought back to strict lint hygiene as well: `cargo fmt` and `RUSTC_WRAPPER= SCCACHE_DISABLE=1 cargo clippy --workspace --all-targets -- -D warnings` now pass across runtime crates, SemOS crates, LSP/test harnesses, xtask, and integration tests, so the SemOS cutover no longer leaves stale all-target drift behind in adjacent packages.
>
> **Downstream workspace/session state (2026-03-25):** `ob-poc` now uses a session-first runtime navigation contract: `Session -> Client Group -> Workspace -> Journey Pack -> Subject/Runbook`. The V2 REPL has an explicit `WorkspaceSelection` gate, workspace-constrained journey routing, and a new `ProductMaintenance` workspace aligned to the design-time `product -> service -> servicing resource -> attribute dictionary` taxonomy. This keeps Semantic OS family/subject/workspace metadata aligned with the actual agent session model rather than a legacy CBU-first route identity.
>
> **Downstream SemOS footprint state (2026-03-25):** the footprint hydration remediation is now closed for the previously unresolved runtime activation verbs. The `service-resource` instance verbs (`provision`, `set-attr`, `activate`, `suspend`, `decommission`, `validate-attrs`) are backed by explicit handler-derived footprints from `resource_ops.rs`, and the generated plugin audit report now closes with `unresolved_binding_count = 0`; `onboarding.auto-complete` remains intentionally classified as `delegated_handler` rather than unresolved.
>
