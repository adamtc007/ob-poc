...
> **Current downstream state (2026-03-11):** `ob-poc` has now cut over to a three-step rip-and-replace utterance pipeline for the SemTaxonomy path: `EntityScope -> EntityState -> SelectedVerb`. Semantic OS remains the governance/contract layer and transition substrate, while `ob-poc` composes proposals from grounded live state plus SemOS-valid verbs. The old SemTaxonomy composition path is removed from normal proposal selection. Current live harness metrics for the new path are `7/176` exact top-verb hits (`3.98%`), `51/176` grounded/stateful responses (`28.98%`), and `35/176` business proposals, which confirms the simplified boundary is in place but still below the legacy business-verb baseline.
>
