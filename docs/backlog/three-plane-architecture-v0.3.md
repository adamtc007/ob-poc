...
> **Status:** Revised proposal absorbing v0.2 peer review. Supersedes v0.1 and v0.2; preserves the destination topology correction in `docs/backlog/sem_os_lift_out_plan.md`.
> **Date:** 2026-04-18
...
**Status:** Pilot DONE — `SemanticStateService` shipped 55b0be16. Pattern established and in use by subsequent composite-blocker slices. Trait catalogue grown to 9 concrete services so far (`SemanticStateService`, `StewardshipDispatch`, `McpToolRegistry`, `LifecycleCatalog`, `AttributeIdentityService`, `ConstellationRuntime`, `TradingProfileDocument`, `SemOsContextResolver`, `SchemaIntrospectionAccess`). Additional traits (`TaxonomyAccess`, `ServiceResourcePipelineService`, `TemplateExpander+DslExecutor`, `TradingMatrixCorporateActions`) still to be defined as their consumer ops are worked.
...
- **Composite-blockers (one trait/blocker per slice, smallest first):**
  ...
  6. `manco_ops` (clean lift — `MancoRoleBridge SQL fn` tracker entry turned out to be a PL/pgSQL `fn_*` function, not a Rust service. 11 ops, all calling DB-side governance fns via sqlx) — commit 41ad4620
  7. `sem_os_audit_ops` (clean lift — the spec entry projected the blocker as "sem_reg audit_op! macros", but on inspection the `audit_op!` macro_rules is local to the file itself, not crate-level. Real dependency was `sem_os_helpers::delegate_to_tool_json` → `sem_reg::agent::mcp_tools::dispatch_tool`. Discovered `ObPocStewardshipDispatch` already cascades non-`stew_` tool names through the general SemReg dispatcher (line 79 of `stewardship_dispatch_impl.rs`), so the 8 audit ops can route through the existing `dyn StewardshipDispatch` trait without a new service-trait entry. Same dispatch_tool helper pattern as `sem_os_governance_ops`. 8 ops, 134 → 142 LOC after strip.) — commit d4243e2e
  8. `observation_ops` + `AttributeIdentityService` trait (1 method: `resolve_runtime_uuid` — narrow projection of the in-crate multi-namespace UNION query over dictionary, registry, SemOS-governed defs. Composite slice was projected as observation_ops + attribute_ops shared, but on inspection `attribute_ops` (2376 LOC) additionally pulls `sem_reg::derivation_spec` + `SnapshotStore` + `sem_reg::types`, requiring multiple traits beyond the identity service — split it to a future slice. observation_ops only needs `resolve_runtime_uuid`, used by 3 of its 5 ops. 5 ops, 1003 → 487 LOC after strip.) — commit 7066288e
  9. `constellation_ops` + `ConstellationRuntime` trait (2 methods: `hydrate` and `summary`, both returning `serde_json::Value` to keep `HydratedConstellation` / `ConstellationSummary` out of the data plane. Spec restart instructions had pointed at view_ops first, but on inspection view_ops pulls 5345 LOC of taxonomy module + 6 cross-plane data types — that's a multi-trait + types-extraction slice, not one-sitting. constellation_ops was the actual cheapest at 141 LOC with one ob-poc dep — both ops return JSON-serializable results so the trait surface stays JSON-shaped at the boundary. 2 ops, 141 → 91 LOC after strip.) — commit d7b1bc9e
  10. `navigation_ops` (clean lift — no new trait. Survey-driven decision per slice #9 meta-lesson: navigation_ops at 500 LOC looked like it had 2 ob-poc deps, but those were only in the legacy `execute(&VerbCall, ...)` paths. After strip, every op is pure JSON-construction with zero DB calls and zero service calls. The Sequencer reads `NavResult` records by message-prefix matching against `ReplResponseV2.message` (`apply_nav_result_if_present`) so the type lives wherever the verbs do — no cross-plane type sharing required. 7 ops, 500 → 326 LOC after strip.) — commit 4c23d8d8
  11. `trading_profile_ca_ops` + `TradingProfileDocument` trait (2 methods: `load_document` and `save_document`, both shaped on `ob_poc_types::trading_matrix::TradingMatrixDocument` — already a boundary type, so no types-extraction. Bridge converts `AstDbError` to `anyhow::Error` at the boundary. 11 ops; both file-local helpers re-parameterised on `&dyn TradingProfileDocument` instead of `&PgPool`. No legacy `execute(&VerbCall, ...)` blocks to strip — file was already migrated. 662 → 599 LOC after strip.) — commit d1e52804
  12. `investor_role_ops` (clean lift — no new trait). Survey-driven decision per slice #10 lesson: the file's 2 ob-poc deps (`crate::dsl_v2::ast::VerbCall` + `crate::dsl_v2::executor::*`) only appeared in legacy `execute(&VerbCall, ...)` paths. After strip, every op is a direct sqlx call to the in-DB `"ob-poc".upsert_role_profile` function (or, for `read-as-of`, a direct query against `investor_role_profiles`). `RoleProfileRow` struct + `From<RoleProfileRow>` for serde_json::Value preserved — still used by `read-as-of`. 6 ops, 868 → 430 LOC after strip (largest legacy-block reduction so far). — commit 02be1aef
  13. `resource_ops` (reuses existing `AttributeIdentityService` from slice #8 — no new trait needed). Pre-flight checklist saved a trait definition: 2 of the 3 ob-poc deps were `crate::dsl_v2::*` legacy-path artifacts, and the 3rd was the in-crate `AttributeIdentityService` which the slice #8 trait already projects. Only `set-attr` (1 of 6 ops) calls `resolve_runtime_uuid`. Refactored `resource_set_attr_impl` to take pre-resolved `attribute_id: Uuid` (caller fetches the trait); other 5 helpers stay `&PgPool`-shaped. 6 ops, 914 → 552 LOC after strip. **First slice to reuse an existing trait without modification.** — commit 60cf4fca
  14. `investor_ops` (clean lift — no new trait, sibling pattern to slice #12 `investor_role_ops`). The 2 ob-poc deps were both `crate::dsl_v2::*` legacy-path artifacts. After strip, every op is direct sqlx (sqlx::query! macro form) against `"ob-poc".investors` and `"ob-poc".holdings`. State-machine validation lives in the `trg_validate_investor_lifecycle` DB trigger. 13 ops, 1361 → ~750 LOC after strip. — commit 1fd12f95
  15. `skeleton_build_ops` (clean lift — the spec's projected `SkeletonBuildOrchestrator` trait turned out unnecessary). The file is self-contained: 1 op (`SkeletonBuildOp`) + 5 long `pub async fn run_*` orchestrator helpers (graph_validate, ubo_compute, coverage_compute, outreach_plan, tollgate_evaluate) called only from within the file. The `pub use run_*` re-exports in ob-poc's mod.rs had ZERO external consumers — dropped. All ob-poc-side imports were `crate::dsl_v2::*` legacy-path artifacts. 1 op + 5 helpers, 1475 LOC, ~30 LOC stripped (legacy execute block + 19 cfg gates). — commit 46c63e06
  16. `team_ops` (clean lift — matrix tag "ob-poc-adapter destination" was wrong, like slice #15 SkeletonBuildOrchestrator). 1 op (`TeamTransferMemberOp`, multi-table atomic membership transfer), only legacy-path artifacts as deps. 171 → 124 LOC. — commit df048399
  17. `sem_os_maintenance_ops` (clean lift — matrix tag wrong again). 7 ops (health-pending, health-stale-dryruns, cleanup, bootstrap-seeds, drain-outbox, reindex-embeddings, validate-schema-sync) — direct sqlx against sem_reg.* and public.outbox. Shared `enqueue_reindex_embeddings` helper preserved (Phase 0g Pattern A remediation). 657 → 343 LOC. — commit f10f2dff
  18. `sem_os_registry_ops` (reuses StewardshipDispatch cascade — slice #7 pattern). 19 of 20 ops route `sem_reg_*` MCP tools via the existing dispatcher; 1 op (`RegistryActiveManifestOp`) does direct sqlx. Same `dispatch_tool` helper shape as slice #7. 20 ops, 455 → 312 LOC. — commit c42b304a
  19. `billing_ops` (clean lift, matrix tag wrong). 14 ops covering fee billing lifecycle. **First slice using a Python strip script** (column-0 `}` close heuristic for legacy impl blocks; brace counter mis-handles JSON literal braces). 14 ops, 2020 → 1076 LOC. — commit ee42b6d5
  20. `capital_ops` (clean lift, matrix tag wrong). 14 ops covering share-class/capital structure. Same strip script as #19; also handled mid-file `use super::helpers::{extract_*}` legacy import. Removed `pub use capital_ops::{...}` re-export block (zero external consumers). 14 ops, 3128 → 1558 LOC. — commit 23c973c5
  21. `deal_ops` (clean lift, matrix tag wrong). 28 ops covering deal lifecycle (initiate, propose, accept, reject, finalize, suspend, terminate; rate cards; deal events). **Largest single file relocated so far.** 28 ops, 4079 → 2280 LOC. — commit f37fb64c
  22. `client_group_ops` (clean lift, matrix tag wrong). 24 ops covering client-group lifecycle. Strip script extended to also delete 8 file-local `fn get_*(verb_call: &VerbCall, ...)` helpers (used only by deleted legacy execute paths). 24 ops, 3489 → ~1740 LOC. — commit 1c5f350b
  23. `discovery_ops` + `entity_kind` + `stategraph` (composite slice — relocated 3 modules together, no new traits). entity_kind (81 LOC, zero deps) and stategraph (514 LOC, only dsl_core dep) were self-contained → relocated alongside discovery_ops instead of trait-wrapping. 4 ob-poc consumers of `crate::entity_kind` updated to `dsl_runtime::entity_kind`. discovery_ops surgery: `gateway_addr()` inlined, `dispatch_tool` calls migrated to StewardshipDispatch (slice #7 cascade trick), `sem_reg_tool` legacy helper deleted. dsl-runtime Cargo.toml gained `dsl-core`, `entity-gateway`, `tonic` deps. 8 ops + 2 helper modules; discovery_ops 3515 → 2774 LOC. — commit 47525f3b
  24. `affinity_ops` + `affinity_graph_cache` + new `SemOsContextResolver` trait. **Methodology shift:** instead of mechanically transliterating the existing Rust (Python regex strip script approach used in #19-#23), this slice was the first to **read the YAML verb contract** (`config/verbs/sem-reg/registry.yaml` for the 6 affinity verbs) and **re-implement fresh in dsl-runtime** against that contract, treating the existing Rust as REFERENCE for SQL + business logic only. Result: 1046 → 565 LOC (vs ~700 LOC if we'd transliterated), much cleaner code without conversion shims (`to_sem_os_actor` JSON round-trip eliminated), wrapper helpers (`load_affinity_graph` 1-line wrapper eliminated), or `crate::sem_reg::*` references (switched to `sem_os_core::abac::ActorContext` + `sem_os_core::types::Classification` directly). Infrastructure: new `SemOsContextResolver` trait (1 method, sem_os_core types directly — no boundary types-extraction); `affinity_graph_cache.rs` relocated with inlined `PgSnapshotRow` decode. 6 ops + 1 helper module; 1 ob-poc consumer (sem_os_schema_ops) updated. — commit 9f0439c8
  25. `sem_os_schema_ops` (13 verbs across 3 dispatch routes) + new `SchemaIntrospectionAccess` trait. Second YAML-first re-implementation slice. Source contract: `config/verbs/sem-reg/schema.yaml` — 5 structure-semantics verbs route via the new `SchemaIntrospectionAccess` trait (6 methods returning `serde_json::Value`); 5 introspect/extract verbs route via the existing `StewardshipDispatch` cascade to the `db_introspect` MCP tool (slice #7 trick + macro-driven boilerplate); 3 diagram verbs use direct sqlx + `sem_os_core::diagram::*` + the relocated `affinity_graph_cache`. Avoids relocating `crate::ontology` (1470 LOC, 8 consumers) and `crate::dsl_v2::verb_registry` (334 LOC, 8 consumers) by projecting only what schema_ops needs onto the JSON boundary. Result: 1059 → 595 LOC. — commit b0d1d333
  26. `view_ops` (15 verbs) + new `ViewService` trait (single-method dispatch). Third YAML-first re-implementation slice. Source contract: `config/verbs/view.yaml`. All 15 `view.*` verbs route through one `dispatch_view_verb(pool, verb_name, args, extensions) -> Value` method — the bridge in ob-poc handles all the heavy lifting against `crate::session::ViewState` + `crate::taxonomy::*` (both 5000+ LOC multi-consumer mega-modules that stay put). Single-method dispatch collapses 15 verb implementations into one trait boundary; the file-local `Selection` state crosses turns through `ctx.extensions["_selection"]` (UUID array), and pending view state stashes in `ctx.extensions["_pending_view_state"]` for the orchestrator to propagate into `UnifiedSessionContext`. 15 ops, 1110 → 122 LOC (thin wrappers only). — commit (prior)
  27. `session_ops` (19 verbs) + new `SessionService` trait (single-method dispatch — same pattern as slice #26). Fourth YAML-first re-implementation slice. Source contract: `config/verbs/session.yaml`. All 19 `session.*` verbs route through one `dispatch_session_verb(pool, verb_name, args, extensions) -> Value` method — the bridge in ob-poc keeps `crate::session::UnifiedSession` (10934 LOC multi-consumer session mega-module) in place. Astro navigation metaphor preserved (universe/galaxy/cluster/system). Pending session state crosses turns through `ctx.extensions["_pending_session"]` (mirrors the legacy `ext_set_pending_session` helper). 19 ops, 1646 → 139 LOC (thin wrappers only); bridge ~900 LOC in `rust/src/services/session_service_impl.rs`. — commit 0663ced3
  28. `service_pipeline_ops` (16 verbs across 7 domains) + new `ServicePipelineService` trait (multi-domain single-method dispatch). Fifth YAML-first re-implementation slice. Source contracts: `config/verbs/service.yaml`, `service-pipeline.yaml`, `service-resource.yaml`, `service-availability.yaml`. The 16 verbs span `service-intent.*`, `discovery.*`, `attributes.*`, `provisioning.*`, `readiness.*`, `pipeline.full`, `service-resource.*` — the trait method takes `(pool, domain, verb, args)` and the macro takes `($struct, $domain, $verb, $rationale)`. Trait returns [`crate::execution::VerbExecutionOutcome`] directly (instead of `serde_json::Value`) because verbs here emit four shapes (Uuid / Record / RecordSet / Affected) per YAML `returns.type` — round-tripping through `Value` would lose the variant tag. Bridge wraps `crate::service_resources::*` (engines, orchestrators, SRDEF registry loader) which stay in ob-poc. 16 ops, 1814 → 152 LOC (thin wrappers only); bridge ~530 LOC. — this commit
  29. `phrase_ops` (9 verbs) + new `PhraseService` trait (single-method dispatch). Sixth YAML-first re-implementation slice. Source contract: `config/verbs/phrase.yaml`. The 9 governed-phrase-authoring verbs route through one `dispatch_phrase_verb(pool, verb, args, principal) -> Value` method — the trait carries `&Principal` (mirroring `StewardshipDispatch::dispatch`) because every snapshot write records `created_by = principal.actor_id` for audit. Bridge wraps `crate::sem_reg::store::SnapshotStore` + `crate::sem_reg::types::*` + `crate::sem_reg::ids::object_id_for` and the embedding-similarity SQL on `verb_pattern_embeddings` / `phrase_bank` / `session_traces` — multi-consumer sem_reg surfaces with no dsl-runtime analogue. 9 ops, 2169 → 99 LOC (thin wrappers only); bridge ~990 LOC. — this commit
  30. `attribute_ops` (16 verbs across 3 domains) + new `AttributeService` trait (multi-domain single-method dispatch with post-execution bindings). Seventh YAML-first re-implementation slice. Source contracts: `config/verbs/attribute.yaml`, `config/verbs/observation/derivation.yaml`. The 16 verbs span 13 `attribute.*`, 2 `document.*` (list-attributes + check-extraction-coverage — `document.list-attributes` re-uses the `attribute.list-by-document` body), 1 `derivation.*` (recompute-stale). Trait returns a wrapping `AttributeDispatchOutcome { outcome, bindings }` — the 3 `define*` verbs publish a snapshot AND need to bind `@attribute` for downstream verbs, so the wrapper iterates `bindings` and calls `ctx.bind(name, uuid)` before returning the outcome. Originally projected as needing a multi-trait split (`AttributeIdentityService` extension + `SnapshotStore` trait + `derivation_spec` trait) — the single-method dispatch pattern dissolves all that complexity since the bridge has direct access to `crate::sem_reg::*` and `crate::services::attribute_identity_service`. **No extension to slice #8's `AttributeIdentityService` was needed.** 16 ops, 2376 → 162 LOC (thin wrappers only); bridge ~1380 LOC in `rust/src/services/attribute_service_impl.rs`. — this commit

**Phase 5a progress: 80 / 89 op files relocated.** Remaining 9 split:
- **3 `ob-poc-adapter` destination** (legitimately stay in ob-poc — verified): `booking_principal_ops` (multi-trait — `crate::api::booking_principal_types`, `crate::database::booking_principal_repository`, `crate::domain_ops::rule_evaluator`), `gleif_ops` (Pattern B — HTTP via `crate::gleif::client`; deferred to Phase 5f), `trading_profile` (3295 LOC, 36 ops including 17 internal component sub-ops; tangles deeply with `crate::trading_profile::*` types — Universe, BookingRule, StandingInstruction, ast_db, document_ops + 200+ LOC of materialize helpers — the bridge would be a 3000-LOC delegation layer, not a real lift; the slice #11 `TradingProfileDocument` trait already projects the load/save surface that `dsl-runtime`-side code actually needs). **Originally 12; ten were re-classified after inspection** (slices #12, #13, #14, #16, #17, #18, #19, #20, #21, #22) — all turned out to be lift-able. The matrix tag was wrong for those, but right for these three.
- **Composite-blocker #31+ candidates** (`dsl-runtime`-destination, each needs ONE trait/dep resolved):
  - **Trait blocker** (needs new `service_traits` entry before move):
    - `SemanticStageRegistry` → `onboarding` + `semantic_ops` (shared — compose; Phase 5c-migrate dependency for `onboarding`)
    - `TemplateExpander` + `DslExecutor` → `template_ops` (Phase 5c-migrate dependency)
    - `TradingMatrixCorporateActions` → `trading_profile` (the `TradingProfileDocument` half was defined in slice #11 and may need extending if `trading_profile`'s ast_db surface widens)
- **A1 Pattern B (deferred to Phase 5f ledger):** `bpmn_lite_ops` (5 ops, gRPC), `source_loader_ops` (16 ops, HTTP), `gleif_ops` (17 ops, HTTP — `ob-poc-adapter` dest regardless). See `docs/backlog/pattern-b-a1-remediation-ledger.md`.

**Restart instructions for a fresh session:** Phase 5a R-sweep YAML-defined dsl-runtime-destination column is **CLOSED**. Remaining 9 files split into Phase 5c-migrate blocked (`onboarding`, `template_ops`), Phase 5f Pattern B (`request_ops`, `bpmn_lite_ops`, `source_loader_ops`, `gleif_ops`), and 3 legitimate ob-poc-adapter destinations (`booking_principal_ops`, `gleif_ops` overlap, `trading_profile`). The natural next step is Phase 5b-deep (typed per-stage Sequencer functions) or Phase 5e (outbox drainer task). Slice #31 trading_profile was investigated and rejected: every verb in the file tangles with `crate::trading_profile::*` types and helpers; the bridge would be a 3000-LOC delegation layer adding ceremony without a real plane separation. The slice #11 `TradingProfileDocument` trait already projects the document load/save surface that other dsl-runtime ops genuinely need. **NEW METHODOLOGY (slice #24):** for verbs with formal YAML contracts (every dsl verb), **re-implement fresh in dsl-runtime against the YAML contract**, treating the existing Rust as REFERENCE only. The Python regex strip script approach used in #19-#23 was fragile (brace counter breaks on JSON literals, import patterns vary per file, file-local helpers leak references) and preserves accidental complexity from the ob-poc impl (helper-shaped indirection, conversion shims, cfg-gate sprawl). Slice #24 produced a 565 LOC clean impl from a 1046 LOC legacy file — much shorter and clearer than transliteration would have produced. **Workflow:** (1) read `config/verbs/<area>/<domain>.yaml` for each verb's args/returns/behavior contract; (2) read existing Rust as reference for SQL + business invariants only (skip all the legacy execute, helper indirection, conversion shims); (3) write fresh op against the contract using local types from the start. All YAML-defined verbs in the `dsl-runtime`-destination column have been lifted. Slices #28-#30 used the single-method dispatch pattern (or its multi-domain variant) from slices #26-#27 to dissolve the multi-trait projections originally projected for `service_pipeline_ops`, `phrase_ops`, and `attribute_ops`. Onboarding/template_ops (DslExecutor blocker), request_ops + bpmn_lite_ops + source_loader_ops + gleif_ops (Pattern B), booking_principal_ops (multi-trait) — defer. Pre-flight checklist: (1) separate true deps from legacy-path artifacts; (2) check trait reuse before defining; (3) check spec-projected traits actually needed; (4) check matrix tags wrong; (5) check whether ob-poc helper module can be relocated alongside (<1000 LOC + ≤1 non-std dep); (6) does StewardshipDispatch cover the surface? (7) one-trait + multi-trait composite split; (8) trait can return serde_json::Value; (9) types in ob_poc_types need no extraction. **(10) Read the YAML contract first — re-implement against it, don't transliterate the Rust** (slice #24 lesson). The file-by-file Python strip script may still be the right tool for verbs without formal YAML contracts, but those should be rare in this codebase.
...
**Status:** 5b-narrow DONE — commit aec638de. **5b-deep scaffold + Stage 1 DONE** — commit ebd13a06. **5b-deep Stage 4 + replay-safety gate DONE** — commit 8524fa1d. **5b-deep all 9 stage contracts DONE** — commit ded3ab4b. **5b-deep wiring DONE** (this commit) — Stages 1, 2a, 2b, 3, 4, 5, 7, 8, 9a, 9b are now wired into the orchestrator's actual call paths emitting `tracing::debug!` events with their typed outputs at each stage boundary. Stages 1 and 4 replaced inline construction (no behavior change); Stages 2a, 2b, 3, 5, 7, 8, 9a, 9b are wired as shadow extractions — they compute the typed output alongside the existing path so the determinism harness has live observability into per-stage data without altering behavior. Stage 6 (gate decision) is the only stage left unwired because no `GatedVerbEnvelope` is yet produced anywhere in the orchestrator — envelope construction is Phase 5c-migrate territory; the typed Stage 6 helper (`GateDecisionOutput::from_envelope`) is ready to consume real envelopes the moment they exist. Stage 9b's wiring lives in `outbox::drainer::OutboxDrainerImpl::run` — per-cycle counters classify rows into done / retryable / terminal buckets and emit a `Stage 9b — drainer cycle summary` debug event. 17 unit tests + 6 outbox integration tests green. The §9.4 determinism harness now has both the typed contract surface AND live emission points to bind fixtures against.
...
**Status:** **5e-foundation DONE.** Migration applied; `public.outbox` table live with the spec §10.4 schema (status CHECK + idempotency unique + pending/processing partial indices). Drainer task implemented at `ob-poc::outbox::OutboxDrainerImpl` with: pre-claim filter on registered consumer effect_kinds, `FOR UPDATE SKIP LOCKED` claim, stale-claim recycler, per-row attempt tracking, max-attempts auto-promote-to-terminal, panic-isolated consumer dispatch, graceful shutdown via `Notify`. First consumer `MaintenanceSpawnConsumer` closes the Phase 0g Pattern A loop (subprocess for `populate_embeddings` reindex). Wired into `ob-poc-web::main` startup. 3 integration tests green: claim→done, retry→max-attempts→terminal, no-consumer-skip.

**5e-narration-cutover DONE (transitional dual-write).** `NarrateConsumer` registered with the drainer; orchestrator's `ReplOrchestratorV2::process()` writes a `Narrate` outbox row after each turn that produces a non-empty narration via `outbox::narration_emit::emit_narration_outbox`. Idempotency keyed on `(trace_id, session_id)` so a re-issued turn can't double-emit. The consumer is a transitional log-only sink — the structural decoupling is complete (the inline narration path could be removed without changing the response contract because the canonical delivery already runs through the outbox), but the inline attach to `ReplResponseV2.narration` remains for current UX continuity. When WebSocket / SSE delivery lands on the frontend, the consumer's `process()` adds the push call and the inline attach is dropped. 2 integration tests green: emit→drainer-claims→done; duplicate `(trace_id, session_id)` is silently deduped at INSERT time via `(idempotency_key, effect_kind)` UNIQUE.

**Replay-safety gate DONE.** `drainer_recovers_stale_claim_after_simulated_worker_crash` integration test exercises the spec §10.4 gate: a row left in `processing` with stale `claimed_at` (simulating a crashed worker) is recycled on the next poll cycle and re-processed cleanly by a fresh drainer. Combined with the assertion that a duplicate INSERT with the same `(idempotency_key, effect_kind)` returns 0 rows affected, this proves the at-least-once delivery + once-observable side-effect property the spec requires. 4 outbox drainer tests + 2 narration tests = 6 green; per-test effect_kind isolation (UiPush / ConstellationBroadcast / Narrate / MaintenanceSpawn / ExternalNotify) keeps the parallel runner from cross-pollinating.
...
| `docs/backlog/sem_os_lift_out_plan.md` | Destination superseded; extraction discipline preserved. |
