...
**Arbitration pass (same day):** the architect ruled on all three escalations raised by the first pass. E-2 and E-3 are CLOSED and applied below (AB1's retirement path now explicitly targets mediation per §15.6, not checkpoint-enforce; AB5's verdict flipped MODEL-SILENT → NONCONFORMANT via the newly-ratified readership rule). E-1 (v0.4 ratification) remains OPEN — blocked on the architect supplying Amendment 1's actual draft text; not fabricated here. AB2's root-cause framing (tier-split, not a missing lock) is now the stated remedy basis for AB2/AB4/AB5 alike.
...
- **E-1**: `EOP-VS-CONTROLPLANE-001 v0.4` (with §15/L1-L3/C1-C3/K1-K3/AB-grounding topology, "Clearing-House Mandate," inverted §8) does not exist in this repository; only v0.3 is checked in. **Ruling: architect's own debt, to be cleared by ratifying §15.5 as R-a (typed read-only lenses) and applying deltas to v0.3 → v0.4.** Blocked pending the architect pasting Amendment 1's actual draft text into-session (same pattern as Addendum C) — the audit prompt's paraphrased section headers are not sufficient to author formal model clauses without risking uncredited fabrication of specification text. Still OPEN.
