...
**Date:** 2026-07-11
**Basis:** `EOP-VS-CONTROLPLANE-001 v0.4` at execution time, now **v0.4.1** (`docs/architecture/EOP-VS-CONTROLPLANE-001_Control-Plane_v0.4.1.md`, ratified same day as a ruling on this report's own escalation E-4). Continuation of MCA-001 (AB1–AB7, the agent conversational boundary) per architect sequencing: "ratify v0.4 → commit the MCA report and the E-3 trace → run the full MCA against the now-in-repo v0.4 → cut the T11 plan."

**Arbitration pass (same day, after Tranche Series T11's plan was drafted against this report):** the architect ruled on escalation E-4 — option (b), C3 tolerates best-effort during checkpoint topology, hardens only at mediation completion (T12). Applied as v0.4.1 (§15.3 amended). C3's verdict below is updated NONCONFORMANT → MIGRATION-PENDING to reflect the ruling; every other finding in this report is unchanged.
...
### C3 — MIGRATION-PENDING (reclassified from NONCONFORMANT; ruling on E-4, v0.4.1)
...
**Ruling (architect, E-4, applied via v0.4.1): C3 is constitutive of mediation completion, not an obligation of checkpoint topology.** Best-effort, non-blocking persistence is the conformant posture through T11; C3 becomes a hard guarantee only when mediation (§15.1) is live and audit-trail reachability holds by construction (the CP is the only path in) rather than by a persistence-layer promise that could silently fail. C3 now shares AB1's MIGRATION-PENDING verdict and T12/mediation terminus — passes the same two-condition test: (a) explicitly identified as transitional by v0.4.1 §15.3's own amended text, (b) tracked with a stated terminus (T12 mediation cutover, per this run's own escalation and the T11 plan's scope ruling). Not a T11 exit criterion.
...
| C3 | MIGRATION-PENDING (ruling on E-4, v0.4.1) | N/A (tracked, T12 terminus) | `src/sequencer.rs:7103` (sole call site); `src/agent/control_plane_shadow.rs:8-9`, `control_plane_envelope_store.rs:228,254,283` (best-effort posture, self-documented, now ruled conformant through T11) | Single call site + explicit best-effort persistence; v0.4.1 §15.3 ruling | Retirement path: T12's mediation cutover, same terminus as AB1. Not a T11 exit criterion — Tranche Series T11's completion invariant (E-T11.A) explicitly carries AB1 and C3 forward as MIGRATION-PENDING with T12-terminating paths |
...
- **E-4**: C3's obligation ("every agent-originated state transition") and the shadow-first posture's own best-effort design principle (established across T9/T10, "never blocks the calling turn") are in tension. **Ruling: RESOLVED — option (b).** C3 tolerates best-effort during checkpoint topology; the hard guarantee is owed only at mediation completion (T12). Applied via v0.4.1 (§15.3 amended), C3's verdict reclassified MIGRATION-PENDING above. CLOSED.
...
**Verdict: NONCONFORMANT.** This run found 8 new NONCONFORMANT topology-layer violations (L1/L2 at BLOCKER severity as structural preconditions for the rest; L3/C1/C2/K1/K2/K3 NONCONFORMANT, C1 BLOCKER-by-necessity) plus 1 new MIGRATION-PENDING (C3 — reclassified same day via the architect's ruling on E-4, applied as v0.4.1; shares AB1's T12/mediation terminus, not a T11 exit criterion). Combined with MCA-001's 4 AB-family NONCONFORMANT + 1 MIGRATION-PENDING findings, the complete MCA-4 register now stands at **12 NONCONFORMANT + 2 MIGRATION-PENDING**, zero unresolved. The mechanism-tier clauses inherited from v0.3 (proof-carrying construction, gate dependency table, B1–B5 discipline) remain fully conformant — every finding this run is in the newly-ratified §15 topology layer, which is expected given v0.4 was ratified same-day with no implementation work yet targeting it.
