...
| **F** — Pattern B A1 (F11) | 5f | **SUBSTANTIALLY ADVANCED 2026-04-22 late-session.** `SemOsVerbOp::pre_fetch(&args, &mut ctx, &sqlx::PgPool)` trait hook added — HTTP/gRPC/subprocess calls that need to produce read-only results now run OUTSIDE the txn scope and merge their result into args. Phase-by-phase status: F.1 (§3.1 bpmn_lite) 4/5 CLOSED (BpmnInspect + BpmnCompile via pre_fetch; BpmnSignal + BpmnCancel via outbox; BpmnStart OPEN — needs outbox for non-idempotent instance-creation); F.2 (§3.2 source_loader) 12/12 CLOSED (all I/O ops); F.3a (§3.3a gleif read ops) 11/17 CLOSED (pure-HTTP + HTTP-with-DB-lookup + dispatcher); §3.3b 6 DB-write-interleaved ops OPEN (need GleifEnrichmentService refactor); F.4 L4 lint `--taint` mode detects helper-indirect violations. | 66b6398b, d57a91be, 0cc4c834, 3760c60f, 94fc11b6, fcc28bdd, 7ee94150, 19ef5035, 3228458f, 6cdbbda1 |
