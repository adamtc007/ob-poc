...
//!
//! Usage:
//!   cargo test --test intent_hit_rate -- --nocapture
//!   cargo test --test intent_hit_rate -- --nocapture exact     # only "easy" tests
//!   cargo test --test intent_hit_rate -- --nocapture hard      # only "hard"+"expert"
//!   INTENT_THRESHOLD=0.60 cargo test --test intent_hit_rate    # override threshold
//!
//! Environment:
        ...
        for (i, case) in tests.iter().enumerate() {
            ...
            let result = match search_result {
                Ok(candidates) => {
                    ...
                    let ambiguity = ob_poc::mcp::verb_search::check_ambiguity(
...
fn classify_outcome(
    ...
    let is_expected = |verb: &str| -> bool {
        verb == expected || alts.iter().any(|a| a == verb)
    };
    ...
    match ambiguity {
        ...
        VerbSearchOutcome::Suggest { candidates: suggestions } => {
...
fn print_summary_report(results: &[TestResult]) {
    let total = results.len();
    let hits = results.iter().filter(|r| r.outcome.is_first_attempt_hit()).count();
    let two_hits = results.iter().filter(|r| r.outcome.is_two_attempt_hit()).count();
    let misses = results.iter().filter(|r| matches!(r.outcome, Outcome::Miss)).count();
    let no_matches = results.iter().filter(|r| matches!(r.outcome, Outcome::NoMatch)).count();
    let errors = results.iter().filter(|r| matches!(r.outcome, Outcome::Error(_))).count();

    println!("=======================================================================");
    println!("  SUMMARY");
    println!("=======================================================================");
    println!("  Total test cases:          {}", total);
    println!("  ✅ First-attempt hits:      {} ({:.1}%)", hits, hits as f64 / total as f64 * 100.0);
    println!("  🟡 Hit w/ clarification:    {}", results.iter().filter(|r| matches!(r.outcome, Outcome::HitWithClarification)).count());
    println!("  🔵 Available in menu:       {}", results.iter().filter(|r| matches!(r.outcome, Outcome::ClarificationAvailable)).count());
    println!("  Two-attempt hit rate:      {} ({:.1}%)", two_hits, two_hits as f64 / total as f64 * 100.0);
...
fn print_category_breakdown(results: &[TestResult]) {
    ...
    let categories = ["direct", "natural", "indirect", "contextual", "adversarial", "multi_intent"];
    for cat in categories {
        let subset: Vec<&TestResult> = results.iter().filter(|r| r.case.category == cat).collect();
        if subset.is_empty() { continue; }
        let hits = subset.iter().filter(|r| r.outcome.is_first_attempt_hit()).count();
        let two = subset.iter().filter(|r| r.outcome.is_two_attempt_hit()).count();
        println!(
            "  {:15} {:3}/{:3} first ({:5.1}%)  {:3}/{:3} two-attempt ({:5.1}%)",
            cat,
            hits, subset.len(), hits as f64 / subset.len() as f64 * 100.0,
            two, subset.len(), two as f64 / subset.len() as f64 * 100.0,
        );
    }
    println!();
}
...
fn print_difficulty_breakdown(results: &[TestResult]) {
    ...
    for diff in ["easy", "medium", "hard", "expert"] {
        let subset: Vec<&TestResult> = results.iter().filter(|r| r.case.difficulty == diff).collect();
        if subset.is_empty() { continue; }
        let hits = subset.iter().filter(|r| r.outcome.is_first_attempt_hit()).count();
        let two = subset.iter().filter(|r| r.outcome.is_two_attempt_hit()).count();
        println!(
...
fn print_domain_breakdown(results: &[TestResult]) {
    ...
    for r in results {
        let domain = r.case.expected_verb.split('.').next().unwrap_or("unknown").to_string();
        let entry = domain_stats.entry(domain).or_insert((0, 0, 0));
        entry.0 += 1; // total
        if r.outcome.is_first_attempt_hit() { entry.1 += 1; }
        if r.outcome.is_two_attempt_hit() { entry.2 += 1; }
    }
    ...
    for (domain, (total, first, two)) in &domains {
        println!(
            ...
            domain,
            first, total, *first as f64 / *total as f64 * 100.0,
            two, total, *two as f64 / *total as f64 * 100.0,
        );
    }
    println!();
}
...
fn print_failures(results: &[TestResult]) {
    ...
    for r in failures {
        ...
        if !r.top_candidates.is_empty() {
            let cands: Vec<String> = r.top_candidates
                .iter()
                .take(3)
...
fn print_latency_stats(results: &[TestResult]) {
    let mut latencies: Vec<f64> = results.iter().map(|r| r.latency.as_millis() as f64).collect();
    ...
    println!("  {:-<68}", "");
    println!("  Avg: {:.0}ms  P50: {:.0}ms  P95: {:.0}ms  P99: {:.0}ms", avg, p50, p95, p99);
    println!();
}
...
async fn build_test_searcher(
    pool: &PgPool,
) -> ob_poc::mcp::verb_search::HybridVerbSearcher {
    use ob_poc::agent::learning::embedder::candle_embedder::CandleEmbedder;
    ...
    use ob_poc::services::VerbService;
    use std::sync::Arc;
    ...
    let embedder = Arc::new(
        CandleEmbedder::new()
            .await
            .expect("Failed to initialize BGE embedder"),
    );
    ...
    println!(
        ...
        stats.phrases_loaded, stats.entity_aliases_loaded
    );

    // Construct searcher with production-equivalent settings
    let mut searcher = HybridVerbSearcher::new(verb_service, Some(learned_data))
