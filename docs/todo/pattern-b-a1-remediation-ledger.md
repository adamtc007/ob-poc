...
| `BpmnCancelOp` | ~~`client.cancel(instance_id, reason)`~~ — REMOVED from verb body | fire-and-forget | **Outbox deferral complete (2026-04-22).** Same pattern as `BpmnSignalOp`. Idempotency key: `bpmn_cancel:<instance_id>:<blake3-16-of-reason>`. Drainer consumer pending (Phase F.1b). | **PARTIAL (verb refactored; consumer pending)** |
