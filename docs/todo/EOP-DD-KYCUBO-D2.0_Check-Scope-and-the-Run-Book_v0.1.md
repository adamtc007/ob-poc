...
**Q2 — CONFIRMED 2026-08-22: out of scope.** This document ships the machinery. What a sanctions, threshold or evidence-sufficiency check actually tests is compliance policy, needs jurisdiction-by-jurisdiction input, and changes constantly once written — precisely the fast-changing half the pack split exists to isolate.
...
**Q4 — WITHDRAWN 2026-08-22 as malformed.** The question asked whether listed-entity relief is a check or an applicability condition. Neither: **listed is a fact about an entity** — alleged, verified, then an attribute on the board like any other, recorded by D1. Checks iterate over the board *independently* of it; listing is an input a check reads, not a switch that removes checks from scope. The "carve-out" is a regulatory outcome, not an architectural mechanism — it is what a beneficial-ownership check concludes when it reads a verified `listed` attribute. It therefore belongs to the check catalogue's content, which Q2 places out of scope. Recorded so it is not reopened as a pending decision.
