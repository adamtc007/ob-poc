...
**Phase 5a progress: 58 / 89 op files relocated.** Remaining 31 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #9. Slice #8 split the projected attribute_ops+observation_ops composite into observation_ops alone (clean lift on a 1-method `AttributeIdentityService` trait); attribute_ops remains pending because it pulls additional `sem_reg::derivation_spec` + `SnapshotStore` + `sem_reg::types` surfaces beyond identity resolution. Next candidates in priority order: (a) `view_ops` behind `TaxonomyAccess` (shape may overlap with `LifecycleCatalog` from slice #5 — design carefully to avoid trait overlap); (b) `discovery_ops` behind `DiscoveryExecutor` + insight builders (larger surface but well-bounded); (c) `phrase_ops` behind `PhraseWatermarkScanner` + embedding similarity; (d) `attribute_ops` (deferred from #8) once a multi-trait composite slice is feasible — likely needs `AttributeIdentityService::resolve_reference` extension PLUS new `SemRegSnapshotStore` and `DerivationSpecCatalog` traits. Onboarding remains blocked by `DslExecutor` on top of `SemanticStageRegistry` (slice #6 inspection) — defer. `sem_os_schema_ops` is matrix-tagged "mixed metadata destination" — defer further. Pre-flight checklist for any candidate: (1) does every import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (2) does the existing `StewardshipDispatch` cascade (slice #7 lesson) already cover the ob-poc surface the file touches? `ObPocStewardshipDispatch` routes general SemReg MCP tools as a fallback arm, so any file that only delegates to named tools (no direct `sem_reg` crate access) may not need a new trait at all. (3) is the projected "shared trait" composite slice actually a one-trait file plus a multi-trait file in disguise? If yes, lift the one-trait file alone (slice #8 lesson) rather than dragging in the harder one.
