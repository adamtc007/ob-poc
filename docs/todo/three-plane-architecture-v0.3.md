...
**Phase 5a progress: 73 / 89 op files relocated.** Remaining 16 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #24. Slice #23 (discovery_ops + entity_kind + stategraph as a composite) demonstrated a new pattern: **when an ob-poc helper module is small and self-contained, relocate it alongside the consumer instead of trait-wrapping**. Avoided 3 traits this way (no DiscoveryExecutor, no StategraphCatalog, no EntityKindMatcher). dsl-runtime gained `dsl-core` + `entity-gateway` + `tonic` Cargo deps. Next candidates (verify with fresh survey first): (a) `sem_os_schema_ops` (1059 LOC) — uses `crate::ontology` + `crate::domain_ops::affinity_graph_cache` + `crate::sem_reg::*`; check whether ontology is self-contained (relocate-able) vs needs trait; (b) `session_ops` (1646 LOC) — needs `SessionLifecycle` trait wrapping `UnifiedSession`; UnifiedSession is centerpiece of 10934 LOC session module — too big to relocate, will need trait; (c) `affinity_ops` (1046 LOC) — needs `AffinityGraph` trait + boundary handling for `crate::sem_reg::ActorContext`/`Classification`; (d) `view_ops` (1110 LOC) — pulls 5345 LOC taxonomy module + 6 cross-plane data types; check whether taxonomy is self-contained (relocate-able) like stategraph was; (e) `service_pipeline_ops` (1814 LOC) — uses `crate::service_resources::*`; check service_resources size + deps; (f) `phrase_ops` (2169 LOC) — heavy `SnapshotStore` use; (g) `attribute_ops` (2376 LOC, deferred from #8). Onboarding/template_ops (DslExecutor blocker), request_ops + bpmn_lite_ops + source_loader_ops + gleif_ops (Pattern B), booking_principal_ops (multi-trait) — defer. Pre-flight checklist: (1) separate true deps from legacy-path artifacts (#10, #12, #14); (2) check trait reuse before defining (#13); (3) check spec-projected traits actually needed (#10, #12, #14, #15-#17); (4) check matrix tags wrong (#15-#22); (5) **check whether ob-poc helper module can be relocated alongside the consumer instead of trait-wrapping** — slice #23 did this for entity_kind (81 LOC) + stategraph (514 LOC); rule of thumb: <1000 LOC + ≤1 non-std dep → relocate; (6) does StewardshipDispatch cover the surface (#7, #18, #23)? (7) one-trait + multi-trait composite split (#8); (8) trait can return serde_json::Value (#9); (9) types in ob_poc_types need no extraction (#11). **For files >1000 LOC use the Python strip script template from #19-#22** — column-0 `}` close heuristic, `[ \t]*#[cfg(feature = "database")]\n` regex, file-local VerbCall helper deletion.
