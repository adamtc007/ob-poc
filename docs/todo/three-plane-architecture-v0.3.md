...
**Phase 5a progress: 63 / 89 op files relocated.** Remaining 26 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #14. Slice #13 (`resource_ops`, 914 → 552 LOC) was the **first slice to reuse an existing trait without modification** — the in-crate `AttributeIdentityService` referenced in `resource_ops` is exactly what the slice #8 trait projects. Pre-flight saved a trait definition. Next candidates by priority (verify with fresh survey first): (a) `affinity_ops` (1046 LOC, 2 deps — verify if real or legacy-path; if real, `AffinityGraph` trait per spec); (b) `view_ops` (1110 LOC, 3 deps but pulls 5345 LOC of taxonomy — multi-slice); (c) `investor_ops` (1361 LOC, 2 deps — likely legacy-path artifacts only, like sibling investor_role_ops slice #12); (d) `skeleton_build_ops` (1475 LOC, 2 deps — needs `SkeletonBuildOrchestrator` per spec); (e) `session_ops` (1646 LOC, 2 deps — needs `SessionLifecycle` + `UnifiedSession`); (f) `service_pipeline_ops` (1814 LOC, 2 deps — needs `ServiceResourcePipelineService`); (g) `phrase_ops` (2169 LOC, 7 deps — heavy SnapshotStore use); (h) `attribute_ops` (2376 LOC, 11 deps — deferred from #8); (i) `discovery_ops` (3515 LOC, 3 deps + StewardshipDispatch cascade fits). Onboarding (DslExecutor) and `sem_os_schema_ops` (matrix-tagged) — defer. Pre-flight checklist: (1) **separate true ob-poc deps from legacy-path-only artifacts** (slices #10, #12); (2) **check whether existing traits already cover the surface** before defining a new one (slice #13 — `AttributeIdentityService` reuse); (3) does every remaining import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (4) does `StewardshipDispatch` already cover the surface (slice #7)? (5) is the composite slice a one-trait + multi-trait file in disguise (slice #8)? (6) can the trait return `serde_json::Value` (slice #9)? (7) are the cross-plane types already in `ob_poc_types` (slice #11)?
