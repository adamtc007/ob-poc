...
**Status:** 5b-narrow DONE — commit aec638de. **5b-deep scaffold DONE** — commit ebd13a06. **5b-deep Stage 4 DONE** — Stage 1 + Stage 4 both extracted. Typed I/O + error enum (`UtteranceReceipt`/`UtteranceInterpretation`/`EntityResolution`/`DagNavigation`/`VerbSurface`/`NlpMatch`/`GateDecision`/`RunbookCompilation`/`DispatchLoop`/`Commit`/`PostCommitDrain` Inputs/Outputs + `StageError` union enum) defined in `rust/src/sequencer_stages.rs`. Stage 1 (`UtteranceReceiptInput::run(now, envelope_version) -> UtteranceReceiptOutput` — clock injected for determinism harness fixtures) wired into `ReplOrchestratorV2::process()` head. Stage 4 (`VerbSurfaceComposition::run() -> VerbSurfaceOutput` — pure transform over the four-case surface enum: `SemOsAvailable`, `SemOsDenyAll`, `SemOsUnavailableWithPack`, `SemOsUnavailableNoPack`) wired into `handle_in_pack` — replaces the inline `mut allowed_verbs / fingerprint / pruned_count` reconstruction at lines 3675–3739 with a pure compositional output. 9 unit tests green (Stage 1 deterministic, no-message no-hash, all 4 Stage 4 composition arms, plus the 4 generic-shape pins). **5b-deep stages 2a, 2b, 3, 5, 6, 7, 8, 9a, 9b OPEN** — incremental extraction work, one stage at a time as `5b-deep-stage-N` slices.
