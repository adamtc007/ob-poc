...
**Phase 5a progress: 60 / 89 op files relocated.** Remaining 29 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #11. Slice #10 (`navigation_ops`) was another survey-driven clean lift — 500 LOC looked like 2 ob-poc deps but those were only in legacy `execute(&VerbCall, ...)` paths; after strip every op was pure JSON-construction. **Always do a fresh `wc -l` + ob-poc-dep survey before picking the next slice — and verify ob-poc deps aren't legacy-path artifacts.** Next candidates by priority (rough — verify with a fresh survey first): (a) `trading_profile_ca_ops` (662 LOC, 1 dep `crate::trading_profile::ast_db`) under `TradingMatrixCorporateActions` trait — only 1 dep though may need types-extraction for the trading-matrix CA structures; (b) `view_ops` under `TaxonomyAccess` (5345 LOC of taxonomy module + 6 cross-plane data types — likely a multi-slice effort, defer if smaller candidate exists); (c) `discovery_ops` (3515 LOC) — large but well-bounded, plus `dispatch_tool` which can route through StewardshipDispatch (slice #7 cascade trick); (d) `phrase_ops` (2169 LOC) under `PhraseWatermarkScanner` + embedding similarity, plus a `SnapshotStore` trait (re-usable for attribute_ops later); (e) `attribute_ops` (deferred from #8, 2376 LOC) once `SnapshotStore` + `DerivationSpecCatalog` traits are feasible. Onboarding remains blocked by `DslExecutor` on top of `SemanticStageRegistry` (slice #6 inspection) — defer. `sem_os_schema_ops` is matrix-tagged "mixed metadata destination" — defer further. Pre-flight checklist for any candidate: (1) **separate true ob-poc deps from legacy-path-only artifacts** — `crate::dsl_v2::ast::VerbCall` and `crate::dsl_v2::executor::{ExecutionContext, ExecutionResult}` only appear in to-be-stripped `execute(&VerbCall, ...)` blocks, so they don't count (slice #10 lesson); (2) does every remaining import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (3) does the existing `StewardshipDispatch` cascade (slice #7 lesson) already cover the ob-poc surface the file touches? `ObPocStewardshipDispatch` routes general SemReg MCP tools as a fallback arm, so any file that only delegates to named tools (no direct `sem_reg` crate access) may not need a new trait at all. (4) is the projected "shared trait" composite slice actually a one-trait file plus a multi-trait file in disguise? If yes, lift the one-trait file alone (slice #8 lesson). (5) Can the trait return `serde_json::Value` instead of an internal type? If both consumer ops wrap their result as `VerbExecutionOutcome::Record(serde_json::to_value(result)?)`, the trait can return `Value` directly (slice #9 lesson).
