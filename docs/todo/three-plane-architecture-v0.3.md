...
- **2 `ob-poc-adapter` destination** (legitimately stay in ob-poc — verified): `booking_principal_ops` (multi-trait — `crate::api::booking_principal_types`, `crate::database::booking_principal_repository`, `crate::domain_ops::rule_evaluator`), `gleif_ops` (Pattern B — HTTP via `crate::gleif::client`; deferred to Phase 5f). **Originally 12; ten were re-classified after inspection** (slices #12, #13, #14, #16, #17, #18, #19, #20, #21, #22) — all turned out to be lift-able. The matrix tag was wrong for these.
