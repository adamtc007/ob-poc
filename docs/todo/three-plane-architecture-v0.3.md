...
**Phase 5a progress: 59 / 89 op files relocated.** Remaining 30 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #10. Slice #9 (`constellation_ops` + `ConstellationRuntime`) was a small-surface clean lift discovered by surveying ALL remaining files instead of trusting the previous restart instructions' priority list — view_ops was projected first but pulls 5345 LOC + 6 cross-plane types. **Always do a fresh LOC + ob-poc-dep survey before picking the next slice — projected priority orders go stale fast.** Next candidates by priority (rough — verify with a fresh survey first): (a) `navigation_ops` (500 LOC, 2 deps) under `NavigationRuntime` / viewport state — needs reading first to confirm trait shape; (b) `view_ops` under `TaxonomyAccess` (shape may overlap with `LifecycleCatalog` from slice #5; will likely need types-extraction for ViewState/Filter/Refinement/Status/Metaphor/TaxonomyContext, so probably a multi-slice effort — defer if a smaller candidate exists); (c) `discovery_ops` (3515 LOC) — large but well-bounded, multiple ob-poc deps including `crate::stategraph`, `crate::entity_kind`, `crate::dsl_v2::gateway_resolver`, plus `dispatch_tool` which can route through StewardshipDispatch (slice #7 cascade trick); (d) `phrase_ops` (2169 LOC) under `PhraseWatermarkScanner` + embedding similarity, plus a SnapshotStore trait (re-usable for attribute_ops later); (e) `attribute_ops` (deferred from #8, 2376 LOC) once SnapshotStore + DerivationSpecCatalog traits are feasible. Onboarding remains blocked by `DslExecutor` on top of `SemanticStageRegistry` (slice #6 inspection) — defer. `sem_os_schema_ops` is matrix-tagged "mixed metadata destination" — defer further. Pre-flight checklist for any candidate: (1) does every import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (2) does the existing `StewardshipDispatch` cascade (slice #7 lesson) already cover the ob-poc surface the file touches? `ObPocStewardshipDispatch` routes general SemReg MCP tools as a fallback arm, so any file that only delegates to named tools (no direct `sem_reg` crate access) may not need a new trait at all. (3) is the projected "shared trait" composite slice actually a one-trait file plus a multi-trait file in disguise? If yes, lift the one-trait file alone (slice #8 lesson) rather than dragging in the harder one. (4) Can the trait return `serde_json::Value` instead of an internal type? If both consumer ops wrap their result as `VerbExecutionOutcome::Record(serde_json::to_value(result)?)`, the trait can return `Value` directly and dodge the entire types-extraction question (slice #9 lesson).
