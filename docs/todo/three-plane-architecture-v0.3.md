...
**Phase 5a progress: 62 / 89 op files relocated.** Remaining 27 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #13. Slice #12 (`investor_role_ops`, pure clean lift — 868 → 430 LOC, largest legacy-block reduction so far) was the second consecutive slice where the survey listed "2 ob-poc deps" but on inspection both were `crate::dsl_v2::*` legacy-path artifacts. **Most files in the survey may turn out to be clean lifts — always inspect actual imports before assuming a trait is needed.** Next candidates by priority (verify with fresh survey first): (a) `resource_ops` (914 LOC, 3 deps — verify deps); (b) `affinity_ops` (1046 LOC, 2 deps — likely needs `AffinityGraph` trait per spec); (c) `view_ops` (1110 LOC, 3 deps but pulls 5345 LOC of taxonomy — multi-slice); (d) `investor_ops` (1361 LOC, 2 deps); (e) `skeleton_build_ops` (1475 LOC, 2 deps — needs `SkeletonBuildOrchestrator` per spec); (f) `session_ops` (1646 LOC, 2 deps — needs `SessionLifecycle` + `UnifiedSession` access); (g) `service_pipeline_ops` (1814 LOC, 2 deps — needs `ServiceResourcePipelineService`); (h) `phrase_ops` (2169 LOC, 7 deps — heavy SnapshotStore use); (i) `attribute_ops` (2376 LOC, 11 deps — deferred from #8); (j) `discovery_ops` (3515 LOC, 3 deps + StewardshipDispatch cascade fits). Onboarding (DslExecutor blocker) and `sem_os_schema_ops` (matrix-tagged) — defer. Pre-flight checklist: (1) **separate true ob-poc deps from legacy-path-only artifacts** — `crate::dsl_v2::ast::VerbCall` + `crate::dsl_v2::executor::*` only appear in to-be-stripped `execute(&VerbCall, ...)` blocks (slices #10, #12); (2) does every remaining import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (3) does `StewardshipDispatch` already cover the surface (slice #7)? (4) is the composite slice a one-trait + multi-trait file in disguise (slice #8)? (5) can the trait return `serde_json::Value` (slice #9)? (6) are the cross-plane types already in `ob_poc_types` (slice #11)?
