...
**Phase 5a progress: 61 / 89 op files relocated.** Remaining 28 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #12. Slice #11 (`trading_profile_ca_ops` + `TradingProfileDocument`) was a survey-driven 1-dep slice that turned out cleaner than projected — `TradingMatrixDocument` was already in `ob_poc_types::trading_matrix` (boundary crate), so no types-extraction was needed. Next candidates by priority (verify with fresh survey first): (a) `team_ops` (171 LOC, matrix-tagged ob-poc-adapter destination — should stay in ob-poc, but **re-verify the matrix tag with the actual deps**); (b) `onboarding` (428 LOC, blocked by DslExecutor — defer); (c) `bpmn_lite_ops` (501 LOC, Pattern B / Phase 5f — defer); (d) `investor_role_ops` (868 LOC, 2 deps); (e) `resource_ops` (914 LOC, 3 deps); (f) `affinity_ops` (1046 LOC, 2 deps); (g) `view_ops` (1110 LOC, 3 deps but pulls 5345 LOC of taxonomy module — multi-slice); (h) `investor_ops` (1361 LOC, 2 deps); (i) `skeleton_build_ops` (1475 LOC, 2 deps); (j) `session_ops` (1646 LOC, 2 deps); (k) `service_pipeline_ops` (1814 LOC, 2 deps); (l) `phrase_ops` (2169 LOC, 7 deps — heavy SnapshotStore use); (m) `attribute_ops` (2376 LOC, 11 deps — deferred from #8); (n) `discovery_ops` (3515 LOC, 3 deps + StewardshipDispatch cascade fits). Onboarding remains blocked by `DslExecutor` (slice #6 inspection) — defer. `sem_os_schema_ops` is matrix-tagged "mixed metadata destination" — defer. Pre-flight checklist: (1) **separate true ob-poc deps from legacy-path-only artifacts** — `crate::dsl_v2::ast::VerbCall` and `crate::dsl_v2::executor::{ExecutionContext, ExecutionResult}` only appear in to-be-stripped `execute(&VerbCall, ...)` blocks (slice #10 lesson); (2) does every remaining import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (3) does the existing `StewardshipDispatch` cascade (slice #7 lesson) already cover the ob-poc surface the file touches? (4) is the projected "shared trait" composite slice actually a one-trait file plus a multi-trait file in disguise? If yes, lift the one-trait file alone (slice #8 lesson). (5) Can the trait return `serde_json::Value` instead of an internal type? If both consumer ops wrap their result as `Record(to_value(result)?)`, the trait can return `Value` directly (slice #9 lesson). (6) **Are the cross-plane types already in `ob_poc_types`?** If yes, no types-extraction needed — the trait can take them directly (slice #11 lesson — `TradingMatrixDocument` was already there).
