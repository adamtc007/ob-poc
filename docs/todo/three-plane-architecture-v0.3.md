...
**Phase 5a progress: 64 / 89 op files relocated.** Remaining 25 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #15. Slice #14 (`investor_ops`, 1361 → ~750 LOC) was a sibling clean lift to slice #12. Affinity_ops was inspected for #14 but found to require at least 2 new traits (AffinityGraph + SemOsContextResolver) plus boundary-type conversion for ActorContext/Classification — deferred. Next candidates by priority (verify with fresh survey first): (a) `affinity_ops` (1046 LOC) — needs AffinityGraph trait for `load_affinity_graph_cached` + a SemOs context resolver trait + boundary-type handling for ActorContext/Classification; multi-trait composite, may also reuse `crate::sem_reg` types; (b) `view_ops` (1110 LOC, multi-slice — 5345 LOC taxonomy); (c) `skeleton_build_ops` (1475 LOC, 2 deps — needs `SkeletonBuildOrchestrator` per spec); (d) `session_ops` (1646 LOC, 2 deps — needs `SessionLifecycle` + `UnifiedSession`); (e) `service_pipeline_ops` (1814 LOC, 2 deps — needs `ServiceResourcePipelineService`); (f) `phrase_ops` (2169 LOC, 7 deps — heavy SnapshotStore use); (g) `attribute_ops` (2376 LOC, 11 deps — deferred from #8); (h) `discovery_ops` (3515 LOC, 3 deps + StewardshipDispatch cascade fits). Onboarding (DslExecutor) and `sem_os_schema_ops` (matrix-tagged) — defer. Pre-flight checklist: (1) **separate true ob-poc deps from legacy-path-only artifacts** (slices #10, #12, #14); (2) **check whether existing traits already cover the surface** before defining a new one (slice #13 — `AttributeIdentityService` reuse); (3) does every remaining import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (4) does `StewardshipDispatch` already cover the surface (slice #7)? (5) is the composite slice a one-trait + multi-trait file in disguise (slice #8)? (6) can the trait return `serde_json::Value` (slice #9)? (7) are the cross-plane types already in `ob_poc_types` (slice #11)?
