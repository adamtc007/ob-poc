...
- **Composite-blocker #6+ candidates** (`dsl-runtime`-destination, each needs ONE trait/dep resolved):
  - **Trait blocker** (needs new `service_traits` entry before move):
    - `AttributeIdentityService` (already defined in slice #8 with 1 method `resolve_runtime_uuid`) extended with `resolve_reference` (full `ResolvedAttributeIdentity` projection) → `attribute_ops`, plus separate traits for the `sem_reg::derivation_spec` + `SnapshotStore` + `sem_reg::types` surfaces it also pulls
