...
**Phase 5a progress: 65 / 89 op files relocated.** Remaining 24 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #16. Slice #15 (`skeleton_build_ops`, 1475 LOC self-contained) was another spec-projection-was-wrong slice — the projected `SkeletonBuildOrchestrator` trait turned out unnecessary because the orchestrator helpers are file-local. With slices #10/#12/#14/#15 all being clean lifts where the spec or initial survey count predicted complexity, **the cheap clean-lift candidates are now exhausted** — the remaining files genuinely need new traits or types-extraction. Next candidates by priority (verify with fresh survey first): (a) `discovery_ops` (3515 LOC) — large but `dispatch_tool` may route through StewardshipDispatch (slice #7 cascade trick, possible reuse like #13's AttributeIdentityService reuse); also pulls `crate::stategraph`, `crate::entity_kind`, `crate::dsl_v2::gateway_resolver`; (b) `session_ops` (1646 LOC) — needs `SessionLifecycle` trait wrapping `UnifiedSession::new()` and `StructureType` enum methods; (c) `affinity_ops` (1046 LOC) — needs `AffinityGraph` trait for `load_affinity_graph_cached` + `SemOsContextResolver` trait + boundary-type handling for `ActorContext`/`Classification`; multi-trait composite; (d) `view_ops` (1110 LOC, multi-slice — 5345 LOC taxonomy); (e) `service_pipeline_ops` (1814 LOC) — needs `ServiceResourcePipelineService` per spec, multiple service_resources types; (f) `phrase_ops` (2169 LOC, 7 deps — heavy SnapshotStore use); (g) `attribute_ops` (2376 LOC, 11 deps — deferred from #8). Onboarding (DslExecutor), `template_ops` (DslExecutor), `sem_os_schema_ops` (matrix-tagged), `request_ops` (Pattern B / bpmn_integration) — defer. Pre-flight checklist: (1) **separate true ob-poc deps from legacy-path-only artifacts** (slices #10, #12, #14); (2) **check whether existing traits already cover the surface** before defining a new one (slice #13); (3) **check whether the spec's projected trait is actually needed** — slices #15 (SkeletonBuildOrchestrator), #14 (none), #12 (none), #10 (NavigationRuntime) all had spec-projected traits that turned out unnecessary; (4) does every remaining import resolve via `dsl-runtime` or via a small trait we can define in one sitting? (5) does `StewardshipDispatch` already cover the surface (slice #7)? (6) is the composite slice a one-trait + multi-trait file in disguise (slice #8)? (7) can the trait return `serde_json::Value` (slice #9)? (8) are the cross-plane types already in `ob_poc_types` (slice #11)?
