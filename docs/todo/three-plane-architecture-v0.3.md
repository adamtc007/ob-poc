...
**Phase 5a progress: 57 / 89 op files relocated.** Remaining 32 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #8. With slice #7 (`sem_os_audit_ops`) closed via the existing `StewardshipDispatch` cascade, the remaining `dsl-runtime`-destination candidates all genuinely require new traits — no more "trait already there in disguise" wins. Cheapest next candidates to evaluate, in this order: (a) `attribute_ops` + `observation_ops` together behind an `AttributeIdentityService` trait (likely the cheapest because the trait surface is small — identity resolution + a write hook — and both ops touch the same surface); (b) `view_ops` behind `TaxonomyAccess` (shape may overlap with `LifecycleCatalog` from slice #5 — be careful not to re-name what already exists); (c) `discovery_ops` behind `DiscoveryExecutor` + insight builders (larger surface but well-bounded); (d) `phrase_ops` behind `PhraseWatermarkScanner` + embedding similarity. Onboarding was inspected at slice #6 time and remains blocked by `DslExecutor` on top of `SemanticStageRegistry` — composing DSL then re-entering the runtime via `crate::dsl_v2::executor::DslExecutor::new(pool)` is not a one-sitting trait, defer until later. `sem_os_schema_ops` uses a broader ontology surface and is matrix-tagged "mixed metadata destination" — probably stays pending until much later. Smaller lift-and-strip files gate on "does every import resolve via `dsl-runtime` or via a 1-method trait we can define in one sitting?"; if yes, move it; if no, define the trait + impl + registration in `ob-poc-web::main` first, then lift. **Important meta-lesson from slice #7:** check whether the existing `dyn StewardshipDispatch` cascade (or any other trait already wired) already covers the ob-poc surface the file touches — `ObPocStewardshipDispatch` routes general SemReg MCP tools as a fallback arm, so any file that only delegates to named tools (no direct sem_reg crate access) may not need a new trait at all.
