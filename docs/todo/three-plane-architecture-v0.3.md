...
**Phase 5a progress: 72 / 89 op files relocated.** Remaining 17 split:
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #23. Slices #19-#22 closed 4 more matrix-tagged "ob-poc-adapter" files via a Python strip script (column-0 `}` close heuristic + cfg-gate `replace_all` + import rewrite + file-local helper deletion). The "ob-poc-adapter destination" set is now down to 2 (booking_principal_ops + gleif_ops). **The truly easy clean-lift candidates are now exhausted.** Remaining 17 files genuinely need new traits, types-extraction, or are legitimately deferred (Pattern B, DslExecutor blocker). Next candidates (verify with fresh survey first): (a) `discovery_ops` (3515 LOC) — `dispatch_tool` may route via StewardshipDispatch (slice #7 cascade trick); also pulls `crate::stategraph`, `crate::entity_kind`, `crate::dsl_v2::gateway_resolver` — multi-trait composite; (b) `sem_os_schema_ops` (1059 LOC) — multi-trait, broader ontology surface (`crate::dsl_v2::verb_registry`, `crate::ontology`, `crate::domain_ops::affinity_graph_cache`, `crate::sem_reg::*`); (c) `session_ops` (1646 LOC) — needs `SessionLifecycle` trait wrapping `UnifiedSession::new()` + `StructureType` enum; UnifiedSession is the centerpiece of a 10934 LOC session module; (d) `affinity_ops` (1046 LOC, multi-trait composite — `AffinityGraph` + `SemOsContextResolver` + boundary handling); (e) `view_ops` (1110 LOC, multi-slice — 5345 LOC taxonomy); (f) `service_pipeline_ops` (1814 LOC) — needs `ServiceResourcePipelineService` per spec; (g) `phrase_ops` (2169 LOC) — needs `SnapshotStore` trait; (h) `attribute_ops` (2376 LOC, deferred from #8). Onboarding/template_ops (DslExecutor blocker), `request_ops` (Pattern B), `gleif_ops` (Pattern B), `bpmn_lite_ops` + `source_loader_ops` (Pattern B), `booking_principal_ops` (multi-trait), 4 ubo files (assumed already relocated check) — defer. Pre-flight checklist: (1) separate true deps from legacy-path artifacts (#10, #12, #14); (2) check trait reuse before defining (#13); (3) check whether spec-projected traits are actually needed (#10, #12, #14, #15, #16, #17); (4) **check whether matrix tags are wrong** (#15-#22 — 8 of 12 ob-poc-adapter tags were wrong); (5) does every remaining import resolve via dsl-runtime or via a small trait? (6) does StewardshipDispatch cover the surface (#7, #18)? (7) one-trait + multi-trait composite split (#8); (8) trait can return serde_json::Value (#9); (9) types in ob_poc_types need no extraction (#11). **For files >1000 LOC use the Python strip script template from #19-#22** — column-0 `}` close heuristic for legacy impl blocks, `[ \t]*#[cfg(feature = "database")]\n` regex (allow leading whitespace) for cfg gates, and explicit pattern matching for file-local `fn name(verb_call: &VerbCall, ...)` helpers.
