...
**Phase 5a progress: 68 / 89 op files relocated.** Remaining 21 split:
- **6 `ob-poc-adapter` destination** (legitimately stay in ob-poc — verified): `billing_ops`, `booking_principal_ops`, `capital_ops`, `client_group_ops`, `deal_ops`, `gleif_ops`. (Originally 12; six were re-classified after inspection: `investor_ops` slice #14, `investor_role_ops` slice #12, `resource_ops` slice #13, `sem_os_maintenance_ops` slice #17, `sem_os_registry_ops` slice #18, `team_ops` slice #16 — all turned out to be lift-able. The matrix tag was wrong for these — they had no actual ob-poc-adapter dependency.)
...
**Restart instructions for a fresh session:** Pick the next composite-blocker #19. Slices #16-#18 closed three matrix-tagged "ob-poc-adapter" files that turned out to be lift-able — the matrix tag was wrong (same lesson as slice #15's SkeletonBuildOrchestrator). The "ob-poc-adapter destination" set is now down to 6 (billing_ops, booking_principal_ops, capital_ops, client_group_ops, deal_ops, gleif_ops). **The truly easy clean-lift candidates are now exhausted** — remaining 21 files genuinely need new traits, types-extraction, or are legitimately deferred. Next candidates (verify with fresh survey first): (a) `discovery_ops` (3515 LOC) — `dispatch_tool` may route via StewardshipDispatch (slice #7 cascade trick); also pulls `crate::stategraph`, `crate::entity_kind`, `crate::dsl_v2::gateway_resolver` — multi-trait composite; (b) `sem_os_schema_ops` (1059 LOC) — uses `delegate_to_tool_json` (5x), can route via StewardshipDispatch like slice #18 sem_os_registry_ops; **inspect first** to confirm the surface; (c) `session_ops` (1646 LOC) — needs `SessionLifecycle` trait; (d) `affinity_ops` (1046 LOC, multi-trait composite); (e) `view_ops` (1110 LOC, multi-slice — 5345 LOC taxonomy); (f) `service_pipeline_ops` (1814 LOC) — needs `ServiceResourcePipelineService`; (g) `phrase_ops` (2169 LOC) — needs `SnapshotStore` trait; (h) `attribute_ops` (2376 LOC, deferred from #8). Onboarding/template_ops (DslExecutor), `request_ops` (Pattern B), 6 `ob-poc-adapter` files — defer. Pre-flight checklist: (1) separate true deps from legacy-path artifacts (#10, #12, #14); (2) check trait reuse before defining (#13); (3) check whether spec-projected traits are actually needed (#10, #12, #14, #15, #16, #17); (4) check whether matrix tags are wrong (#15-#18); (5) does every remaining import resolve via dsl-runtime or via a small trait? (6) does StewardshipDispatch cover the surface (#7, #18)? (7) one-trait + multi-trait composite split (#8); (8) trait can return serde_json::Value (#9); (9) types in ob_poc_types need no extraction (#11).
