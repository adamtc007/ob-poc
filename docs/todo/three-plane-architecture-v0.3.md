...
**Status:** 5b-narrow DONE — commit aec638de. **5b-deep scaffold DONE** — commit ebd13a06. Typed I/O + error enum (`UtteranceReceipt`/`UtteranceInterpretation`/`EntityResolution`/`DagNavigation`/`VerbSurface`/`NlpMatch`/`GateDecision`/`RunbookCompilation`/`DispatchLoop`/`Commit`/`PostCommitDrain` Inputs/Outputs + `StageError` union enum) defined in `rust/src/sequencer_stages.rs`; Stage 1 extracted as a pure helper (`UtteranceReceiptInput::run(now, envelope_version) -> UtteranceReceiptOutput` — clock injected for determinism harness fixtures) AND wired through `ReplOrchestratorV2::process()` head as shadow extraction (no behavior change; the inline trace-store write still owns the side effect, the typed call gives the harness a deterministic Stage 1 output to pin against fixtures). 5 unit tests green. **5b-deep stages 2a–9b OPEN** — incremental extraction work, one stage at a time as `5b-deep-stage-N` slices.
