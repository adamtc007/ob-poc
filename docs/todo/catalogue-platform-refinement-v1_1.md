# Catalogue Platform Refinement — Vision & Scope (v1.1)
...
> **v1.0 changes from v0.9 (retained for history):** two final edits. (1) Resolved an internal contradiction around P-G and Tranche 1 completion — the strict position is adopted: P-G (named authority for tier-assignment governance) is part of Tranche 1 DoD item 11 and §11 stopping points, so Tranche 1 is not complete until P-G is resolved. The §13 escape hatch ("Tranche 1 can still exit with the other eight artefacts") is removed. (2) Effective-tier pseudocode in P11 now explicitly handles the empty-escalation-rules case.
...
The catalogue is canonical; the declared DAG taxonomy is a governed projection artefact — **the architectural end-state**. In current ob-poc that artefact is only partially present (see §4.2 for the accurate state). Tranche 1 Phase 1.3 and Tranche 2 Phase 2.A are the phases that bring declared DAG taxonomies into existence for every workspace.
...
- Workspace DAG model with (formerly ESPER) navigation and four primary workspaces (Deal, CBU, KYC, Instrument Matrix) plus auxiliary workspaces (OnBoarding, ProductMaintenance, SemOsMaintenance).
- ~1,500 DSL verbs across 134 domains.
- **`SemOsVerbOp` plugin surface** — the post-Phase-5c-migrate execution contract. Approximately 686 impls total, split across two registries (`sem_os_postgres::ops::*` ≈ 567 impls, `rust/src/domain_ops/*` ≈ 119 Pattern-B impls) and wired via `build_registry() + extend_registry()` in `ob-poc-web::main`. (v1.0 cited "625 `CustomOperation` implementations"; the `CustomOperation` trait + inventory registry + `dsl-runtime-macros` crate were deleted 2026-04-22 in Phase 5c-migrate slice #80. `SemOsVerbOp` is now the sole plugin-verb execution contract.)
...
- **`trading_profile_lifecycle` state machine** is declared today on the `trading_profile` slot of the Instrument Matrix `trading_streetside` constellation. This is the only formally declared state machine across the four primary workspaces; see §4.2 for the scope of what is still absent.
...
- Declared DAG taxonomies as explicit artefacts for every workspace. (Instrument Matrix has **one** state machine declared — `trading_profile_lifecycle` — covering one of ~18 slots across three constellation maps. Deal, CBU, KYC have none declared as formal YAML artefacts. Tranche 1 Phase 1.3 produces the full set.)
