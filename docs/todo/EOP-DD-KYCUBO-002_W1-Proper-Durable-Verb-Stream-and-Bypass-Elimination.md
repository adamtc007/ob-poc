...
| 0.4 | 2026-07-15 | **D3 reversed — `ubo.board-controller.override` deleted, not completed.** The verb shipped appending to the stream but the fold half described above (composing derived-controller + override-event, "latest non-superseded override wins") was never built — `apply_one_control_event` had no match arm for it, so it had zero effect on `ControlState` from the day it landed. On review the verb's purpose was no longer clear enough to justify finishing the fold, so the owner decision was to remove the verb entirely (op, YAML, pack admission, test, registration) rather than complete D3's original design. See `docs/research/control-plane-ownership-ledger.md` (2026-07-15 entry) for the removal record. If board-control override capability is needed again, this doc's original D3 design (this row and the 0.3 entry above) is the starting point — the `board_controller_overrides`/`cbu_board_controller`/`board_controller_cache` tables it was meant to replace are still gone (empty, per the W4/W7 legacy-verb rip), so a fresh design would need to re-derive the projection shape, not just re-add the verb. |
