...
### Version: v0.4 (all three architect decisions ratified)
### Date: 2026-07-13
### Status: AD-1 RESOLVED as (a) consume-time validity (architect, 2026-07-13); AD-2 RESOLVED as (b) per-(verb, path) dimension (architect, 2026-07-13); AD-3 RESOLVED as (a) hold-the-merge (architect, 2026-07-13). All three architect decisions are now ratified — no tranche in this plan is blocked on an open AD. PIR verdict RATIFY-WITH-AMENDMENTS, all six amendments (GRADPLAN-D-001..006) applied as of v0.2.
...
ratified; the brackets mark the deliverable design docs those
ratifications unblocked, not-yet-written, as the remaining gate):

```
G0 (hygiene + docs; no merge)
 ├─► G1 [seal→consume doc: blocks item 2 / exit only] ─┐
 ├─► G2 ────────────────────────────────────────────────┼─► GM (merge+deploy, once)
 ├─► G3 [enforcement-dimension doc, AD-2(b) ratified]   │        │
 │      └─► G4 ─────────► G5 ─► (E3 matrix              │        ▼
...
Bracket tags mark what the design doc actually blocks (GRADPLAN-D-004
principle, now applied to the ratified ADs' successor artifacts): G1's
design doc (item 1) and items 3–4's grind prep can start now that
AD-1(a) is settled; only item 2 and the tranche exit wait on the doc
itself. G3's design doc is the concrete mechanical spec AD-2(b) still
needs (path-tag enum, keying, env-var syntax) — resolving AD-2 did not
write it.
...
### G1 — Path A enforce-readiness: seal → handle → consume [AD-1(a) ratified]
...
### G3 — Enforcement-dimension mechanical design [AD-2(b) ratified]
...
- **Design docs are the critical path** (all three ADs ratified,
  2026-07-13) — G1's seal→consume doc, G3's enforcement-dimension doc,
  G2b's audit-stream doc, and G2 item 4's provenance-dimension work are
  the genuine bottleneck now: every architect-decision fork is closed,
  but none of the four deliverables that turn those decisions into
  code exists yet. G1 item 2 and G4's first line both wait on their
  respective doc, not on further decisions.
