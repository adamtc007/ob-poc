...
## 1.1 ✅ Batch EntityRef Resolution [DONE]

**Status**: DONE - Implemented 2025-12-14

**What was done**:
- `batch_resolve_all_refs()` now called in `validate()` after parsing
- Cache threaded through `validate_verb_call` → `validate_argument_value`
- Cache lookup before falling back to individual gRPC calls
- ~6x speedup (30 refs → 5 gRPC calls instead of 30)

---

## 1.2 ✅ Fix `ubo.verify-ubo` Wrong Table Reference [DONE]
...
## 1.3 ✅ Fix `isda.create` Missing Counterparty Lookup [DONE]
...
## 1.4 ✅ Fix KYC Case Status Enum Mismatch [DONE]
...
## 1.5 ✅ Fix UBO Verification Status Enum Mismatch [DONE]
...
## 2.1 ✅ Consolidate Duplicate CBU Category Constraint [DONE]
...
## 2.2 ✅ Add Missing `cbu.attach-evidence` Verb [DONE]
...
## 2.3 ✅ Add Missing `ubo.supersede` and `ubo.remove` Verbs [DONE]
...
## 2.4 ✅ Add Missing `kyc-case.reopen` Verb [DONE]
...
## 3.1 ✅ Create `diagnostics.rs` - Unified Diagnostic Types [DONE]
...
## 3.2 ✅ Create `execution_result.rs` - Clean Result Types [DONE]
...
## 3.3 ✅ Create `planning_facade.rs` - Central Planning Entrypoint [DONE]
...
## 3.4 ✅ Create `repl_session.rs` - REPL State with Undo [DONE]
...
## 4.1 ✅ Create `ops.rs` - Op Enum Definition [DONE]
...
**Dependency Rules**:
- `EnsureEntity` → no deps (creates entity)
- `SetFK(A.fk = B)` → depends on `EnsureEntity(A)`, `EnsureEntity(B)`
- `LinkRole(cbu, entity, role)` → depends on both entities
- `AddOwnership(owner, owned)` → depends on both entities
- `Materialize(profile)` → depends on `UpsertDoc(profile)` + referenced entities

---

## 4.2 ✅ Create `compiler.rs` - Verb → Op Compiler [DONE]
...
## 4.3 ✅ Create `dag.rs` - DAG Builder + Toposort [DONE]
...
├── ast.rs                    # AST types
├── parser.rs                 # Parser
├── diagnostics.rs            # NEW: Unified diagnostic types
├── execution_result.rs       # NEW: StepResult, ExecutionResults
├── planning_facade.rs        # NEW: analyse_and_plan entrypoint
├── ops.rs                    # NEW: Op enum
├── compiler.rs               # NEW: Verb → Op compiler
├── dag.rs                    # NEW: DAG builder, toposort
├── execution_plan.rs         # ExecutionPlan, PlanningContext
├── verb_registry.rs          # UnifiedVerbRegistry
├── binding_context.rs        # BindingContext
├── repl_session.rs           # NEW: ReplSession with undo
├── topo_sort.rs              # Existing (may merge with dag.rs)
├── executor.rs               # DslExecutor (updated for DAG)
├── generic_executor.rs       # GenericCrudExecutor
├── gateway_resolver.rs       # EntityGateway client
├── semantic_validator.rs     # Validation (uses batch resolution)
├── custom_ops/
│   ├── mod.rs                # CustomOperation trait, registry
│   └── ...                   # Individual implementations
├── lsp_validator.rs          # LSP handler (uses facade)
└── synthetic_steps.rs        # NEW: build_synthetic_create helper
```
