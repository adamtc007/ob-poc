...
> **Verbs:** 1,281 canonical verbs across 134 domains; 19 are dsl.kyc stream-backed under the four-segment `kyc_ubo.<capability>.<domain>.<intent>` scheme (RATIFIED 2026-08-22 — supersedes the old `ubo.*`/`kyc.*` three-segment names below): `kyc_ubo.assert.edge.*` (6), `kyc_ubo.assert.subject.*` (6), `kyc_ubo.decide.determination.*` (1 — `freeze`; the domain's other 3 pre-rename verbs, `select-strategy`/`compute-fold`/`apply-smo-fallback`, are retired), `kyc_ubo.assert.entity.*` (3 — `screening`/`identity`/`risk`, relocated here from the old `kyc.obligation.update-*` names by TS.6 §3), `kyc_ubo.decide.subject.*` (2 — `approve`/`reject`, the old `kyc.person.*`), `kyc_ubo.decide.obligation.*` (1 — `waiver`, the old `kyc.obligation.waive`; `create`/`satisfy` DISSOLVED by D2.0 §5, no reintroduction path); `kyc.role.assign`/`kyc.role.withdraw` (W3, pre-rename name — no four-segment form exists) retired 2026-08-12 (T0.3 K-G7 — fold-blind write, same defect class as `ubo.board-controller.override`; reintroduction only via T6 with a precondition attached from birth); `ubo.board-controller.override` (pre-rename name — no four-segment form exists) removed 2026-07-15 (D3's own doc comment said it was meant to replace a `board_controller_overrides` table that never existed; it had zero fold effect — appended to the stream, nothing ever read it back — and its purpose was no longer understood, so it was deleted rather than left as unexplained dead capability); 58 legacy determination verbs deleted (ubo/control/ownership/board write verbs replaced)
...
> **KYC/UBO dsl.kyc V&S implementation:** `docs/todo/EOP-DD-KYCUBO-001_*.md` + `EOP-DD-KYCUBO-002_*.md` + `EOP-DD-KYCUBO-003_*.md`. W1–W7 stream/lexicon/projection infrastructure complete on branch `codex/phase-1-5-governance-closure`; **EOP-DD-KYCUBO-003 (2026-07-01)** wired the actual determination logic into the write path — `kyc_ubo.decide.determination.freeze` (then named `ubo.determination.freeze`, pre-2026-08-22 rename) had been bypassing `OwnershipProngStrategy` entirely (a naive all-active-edges proxy, no threshold/axis split) and `kyc_ubo.decide.subject.approve` (then `kyc.person.approve`) had no K-23 gate; both fixed, RED→GREEN proven. **M4 (2026-07-15): `ControlProngStrategy` landed** — `DeterminationStrategy::resolve()` generalized to take `&ControlState` (was a pre-filtered economic-edge slice); the new strategy resolves natural persons via a chain of control-kind edges (voting rights, board appointment, GP statutory, LLP designated member, dominant influence — `EdgeKind`), `Prong::ControlByOtherMeans`, no percentage quantum. Selectable via `select-strategy`'s `strategy: control_prong_strategy` (the `select-strategy` verb — then `ubo.determination.select-strategy` — was itself retired by TS.6 P2, no four-segment form exists); `freeze` dispatches to it alongside `ownership_prong_strategy`. Found and fixed while wiring the live path: `kyc_ubo.assert.edge.control`'s (then `ubo.edge.assert-control`) YAML arg was named `edge_kind` but the fold reads payload key `kind` — every real DSL call would have silently misclassified every control edge as `DominantInfluence` (same defect class as the R3 structure-class bug from EOP-DD-KYCUBO-003, caught before this verb ever went live). **TS.0-TS.4 (2026-08-12/08-14, `EOP-DD-KYCUBO-KIT-TS0_Structure-Class-Strategy-Design_v0.1.md`, RATIFIED 2026-08-12) closed the rest of M2/M4**: all 11 `StructureClass` variants now have a real `DeterminationStrategy` (`IMPLEMENTED_STRATEGY_CLASSES` is total) — `trust_role_strategy` (trustee/protector always, settlor unless proven irrevocable, beneficiary never), `fund_control_strategy` + `foundation_council_strategy` (corporate-form fund/foundation control sits with manager/council, not investors), `state_owned_strategy` + `cooperative_member_strategy`, and `nominee_pierce_strategy` (K-8 — the pierce-nominee verb, then `ubo.edge.pierce-nominee`, supersedes the nominee edge and asserts the disclosed nominator's real kind in one governed event; `freeze` hard-errors while any unpierced `EdgeKind::Nominee` edge remains active — pierce-nominee was itself later retired as a standalone verb by TS.6 P2, folded into a macro composing `kyc_ubo.assert.edge.control`'s pierced-from arg + `kyc_ubo.assert.edge.supersession`). The `assert-control` `kind`→`EdgeKind::TrustRole` sub-kind wire mapping is closed (`EDGE_KIND_WIRE_VALUES`, 11 strings incl. all 4 `trust_*` sub-kinds, fail-closed normalizer). Any string outside the 8 named strategies still fails loudly at `freeze`, not silently — that discipline is unchanged, it now just has nowhere left to fall through to for the 11 known classes. Remaining, explicitly out-of-scope: crossing into the economic axis to resolve an intermediate controlling entity's own UBOs (v2, documented on `ControlProngStrategy`, not a gap in v1). Crate stack: `ob-poc-kyc-substrate` (pure engine, no sqlx) ← `ob-poc-kyc-store` (Postgres membrane, §3 append protocol, projectors, drainers, W2 manifest publish) ← `ob-poc-kyc-seam` (the only KYC crate touching dsl-runtime; `append_in_scope` §3.6 chokepoint). `as_of` is now a field on `VerbExecutionContext` (frozen at dispatch). Dep-gate: `rust/scripts/check_kyc_substrate_deps.sh`.
...
**KYC/UBO dsl.kyc V&S Program (EOP-VS-KYCUBO-001 v0.6 "From Percentage to Determination") — W1–W7 infrastructure COMPLETE (2026-06-30), determination logic corrected 2026-07-01 (EOP-DD-KYCUBO-003):** Three-crate pure stack (`ob-poc-kyc-substrate` ← `ob-poc-kyc-store` ← `ob-poc-kyc-seam`). W1 durable verb stream: `kyc_intent_events` (K-16 system of record, per-subject ordering Q6, `committed_at DEFAULT clock_timestamp()` B1), §3 append protocol (FOR UPDATE per-subject lock, seq from `next_seq` not a SEQUENCE — gap-free under rollback), `FoldRegistry` (D2 — total per-event dispatch on `lexicon_hash`, hard-error on unregistered hash). W2 reference-plane lexicon: `dsl_verbs.lexicon_hash` + `kyc_lexicon_manifest` (whole-manifest hash Q7, `publish_manifest()`, K-30 lint — all dsl.kyc verbs covered, including the W5 obligation/person verbs, T6.0 closed 2026-08-12/2026-08-17). W4 determination: 12 dsl.kyc verbs, current four-segment names (RATIFIED 2026-08-22) — `kyc_ubo.assert.edge.{control,economic-interest,evidence,verification,supersession,reconciliation}` + `kyc_ubo.decide.determination.freeze` (`select-strategy`/`compute-fold`/`apply-smo-fallback` sibling verbs since retired) + `kyc_ubo.assert.subject.{register,structure-class}` — 58 legacy write verbs (ubo/control/ownership/board) deleted; tables `control_edges`/`entity_ubos`/`cbu_board_controller` were empty (no active write path). W7 migration: `ubo.compute-chains` deleted, `ownership_prong_strategy` substrate. W3 role-basis verbs `kyc.role.{assign,withdraw}` **retired 2026-08-12** (T0.3 K-G7 fold-blind write; K-21's "obligation basis recorded, never inferred" is already fully met by the create verb's required `role` arg) — retired before the four-segment rename, no `kyc_ubo.*` form exists. W5 obligation lifecycle, current names: `kyc_ubo.decide.obligation.waiver` + `kyc_ubo.assert.entity.{identity,screening,risk}` (relocated from the pre-rename `kyc.obligation.update-*` names by TS.6 §3) + `kyc_ubo.decide.subject.{approve,reject}`; `create`/`satisfy` DISSOLVED by D2.0 §5 (no reintroduction path) — parallel tracks Q4; existing `screening.*`/tollgate consumed as inputs, not replaced (V&S §9.2). W6 projections: `kyc_control_edge_projection` + `kyc_obligation_projection` + `kyc_subject_rollup_projection` — disposable folds (K-34), `PgKycProjectionDrainer` + `PgKycObligationDrainer` (FOR UPDATE SKIP LOCKED, per-effect-kind, idempotent/convergent). `as_of` on `VerbExecutionContext` (frozen at dispatch). **EOP-DD-KYCUBO-003 (2026-07-01) — determination-logic and approval-gate remediation:** a fresh code-level re-verification found `freeze` (then `ubo.determination.freeze`, now `kyc_ubo.decide.determination.freeze`) was bypassing `OwnershipProngStrategy` entirely — it resolved every distinct edge-source as a candidate with no threshold, no economic/control axis split, and no structure-class dispatch (a hand-rolled proxy, the substrate strategy was only exercised by an isolated unit test) — and `person.approve` (then `kyc.person.approve`, now `kyc_ubo.decide.subject.approve`) had no K-23 gate (approved unconditionally). Both fixed: `freeze` now dispatches to the real `DeterminationStrategy` selected via `classify-structure`/`select-strategy` (then `kyc.subject.classify-structure`/`ubo.determination.select-strategy`, now `kyc_ubo.assert.subject.structure-class`; `select-strategy` itself later retired, no four-segment form) (fails loudly, not silently, for any structure class beyond `ownership_prong_strategy`, which is the only one implemented), applies a threshold (`threshold-pct` arg, default 25.0 pending a governed reference-plane table), and records candidates/basis/prong on the freeze event payload (K-1/K-35); `person.approve` now folds obligations and rejects unless `SubjectOverallState::AllTerminal` (K-23). Also fixed in the same pass: a silent `structure-class`/`structure_class` payload-key bug that left `ControlState.structure_class` always `None`, and freeze's lexicon precondition check being dead code (`stream_append` passed `None` instead of its own FQN, so K-14 reconcile-before-fold never actually ran). RED→GREEN proven via `git stash` (`rust/tests/kyc_m3_remediation.rs`, 4 tests). Still open (M2/M4, unauthorised): lexicon-manifest coverage for the 8 remaining obligation/person verbs (T6.0), the pierce-nominee move (then `ubo.edge.pierce-nominee`, since retired as a standalone verb, K-8 — nominees are not currently pierced), and a real control-prong strategy for fund-LP/LLP/trust structure classes (Success Criterion 2 unproven against the live path). End-to-end: substrate 19/19, store 13/13, `rust/tests/kyc_*.rs` all green, plugin-coverage PASS, dep-gate PASS. **`kyc_stream` `SourceOfTruth` variant DONE (2026-08-17):** added to dsl-core's `SourceOfTruth` enum (`github.com/adamtc007/dsl`, `refactor/sem-os-pack-policy` @ `1965253`, pushed; ob-poc pin bumped `fa08c0e0`); the 13 dsl.kyc verbs in `config/verbs/kyc/dsl-kyc.yaml` now declare `source_of_truth: kyc_stream` instead of the `workflow`-with-TODO stopgap. `source_of_truth` is descriptive metadata excluded from the verb contract hash (`verb_sync.rs::compute_verb_hash`), so this was a pure metadata correction — `verbs check` reported 0 hash mismatches, no embeddings repopulation needed. **W5 screening hook wiring DONE (2026-08-17, commit `b028e5b3`):** `kyc_ubo.assert.entity.screening` (then `kyc.obligation.update-screening`) was real and fold-wired but nothing called it — `screening.complete`/`review-hit` were plain CRUD writes with a descriptive-only `emits_event` field consumed by no runtime dispatcher. Both moved from `behavior: crud` to `behavior: plugin` (`ScreeningComplete`/`ScreeningReviewHit`, `src/domain_ops/kyc_stream_ops.rs`), preserving the original UPDATE semantics exactly, then in the same transaction resolve the screened entity (the dsl.kyc verbs' `subject-id` = the entity's own real UUID, confirmed via `tests/kyc_w3_w5_w6.rs` — no new linkage table needed), fold its current obligation state, and fan out one `kyc_ubo.assert.entity.screening` event per open obligation (`CLEAR`→satisfied, `HIT_PENDING_REVIEW`/`ERROR`→in_progress, `HIT_CONFIRMED`→rejected, `HIT_DISMISSED`→satisfied; fail-closed on any other status). Tests: `rust/tests/kyc_w5_screening_hook.rs` (3 cases); RED-proof was `dead_code = "deny"` refusing to compile with the fan-out call removed, not just a failing assertion. Remaining V&S scope (separate program, plan v0.1 at `docs/todo/EOP-DD-KYCUBO-004_W5-Screening-Hook-and-W6-Projection-Cutover-Plan_v0.1.md`, design v0.1 at `docs/todo/EOP-DD-KYCUBO-005_W6-Case-Workstream-Fold-Design_v0.1.md`, neither yet implemented): **W6 case/workstream projection cutover** — `cases`/`entity_workstreams` still written directly, by 14 verbs (not 3, per the 005 doc's corrected inventory — `kyc-case.yaml`/`entity-workstream.yaml` declare `behavior: crud` writers too, invisible to a hand-written-SQL grep) across 4 trigger domains (kyc-case, entity-workstream, request, red-flag-cascade), unlike control edges and obligations which already fold off the stream. The 005 doc found `cases`/`entity_workstreams` are FK-referenced by ~25 other tables (several `ON DELETE CASCADE`), which rules out the 004 doc's original "new projection table behind a compatibility view" shape (a view can't be an FK target) — revised design: the drainer UPSERTs the existing tables in place, case_id is the stream subject (one stream per case, workstreams disambiguated inside it), `case_ref`'s trigger-generated sequence becomes a captured effect at `kyc.case.open` append time for fold-replay determinism, and no `ob-poc-kyc-seam` API change is needed (case/workstream verbs do their own pre-append reads, same as every existing dsl.kyc verb). Full phased cutover (dual-write burn-in before any direct writer retires) in the 005 doc; still design-only, no code written. Full plan + execution log: `docs/todo/EOP-DD-KYCUBO-003_Determination-Logic-and-Approval-Gate-Remediation.md`. All work on branch `codex/phase-1-5-governance-closure` (not yet merged to main).
...
**Selector-arg `valid_values` is mandatory, always the wire string, not a description-only enum** (2026-07-15): every selector/strategy arg dispatched via `selector_dispatch` — or matched via a hand-rolled `match` on a string arg, like `kyc_ubo.decide.determination.freeze`'s (then `ubo.determination.freeze`) `strategy` dispatch on `select-strategy`'s recorded value — MUST declare `valid_values` in the verb YAML using the *exact* runtime match string, not a prose description of it. Two real bugs found by auditing `kyc_ubo.assert.subject.structure-class`/`select-strategy` (then `kyc.subject.classify-structure`/`ubo.determination.select-strategy`; `select-strategy` itself later retired, no four-segment form exists) for this after building `selector_dispatch`: (1) `structure-class` had no `valid_values` at all — the 11-variant `StructureClass` taxonomy (`ob-poc-kyc-substrate/src/fold/control.rs`) existed only in Rust, invisible at the config layer the way CBU's `role-type` `valid_values: [ROLE, OWNERSHIP, ...]` is; (2) `strategy`'s description said `"ownership_prong or smo_fallback"` while the only real `DeterminationStrategy::name()` is `"ownership_prong_strategy"` — a caller or an LLM doing arg extraction from the description alone would pass a string that fails at `freeze` time. `valid_values` is descriptive metadata (feeds Sage arg extraction + SemOS `AttributeDataType::Enum` projection, not a DSL-parse-time hard reject — confirmed via `crates/sem_os_obpoc_adapter/src/scanner.rs:977`), so widening it costs nothing; the risk is entirely on the side of leaving it absent or wrong. When a selector's valid range includes not-yet-implemented arms, list them anyway and say so in `description` — the taxonomy is real even when the strategy behind an arm isn't built yet, that's a different, separately-tracked gap. (`structure-class`'s `llp`/`trust`/fund classes were exactly this at the time this rule was written — M4 was open; TS.0-TS.4 has since closed all 11 `StructureClass` arms, see KYC/UBO section above.)
