...
> **SessionVerbSurface:** ✅ Complete + Group Scope + Composite State Bias - 6-step compute pipeline (was 8): Registry → AgentMode → Scope+Workflow (merged) → SemReg CCIR → Lifecycle → Rank+CompositeStateBias. Group scope gate (`has_group_scope`): no group = bootstrap domains only. `GroupCompositeState` loads group-level UBO/control + per-CBU onboarding state (case/screening/docs) from DB, derives `next_likely_verbs` with layered DAG: Group UBO → CBUs → Case → Screening → Docs → Done. Redundant post-search re-filtering removed (was 4x SemReg check, now 1x). `VerbOption.suggested_utterance` on disambiguation responses. 17+4 unit tests
> **Loopback Calibration (v0.3):** ✅ Infrastructure complete — 11 Rust modules (`rust/src/calibration/`), 2 migrations, 11 CLI commands (`cargo x calibration`), 16 scenario packs (139 utterances) in `artifacts/calibration/`, embedding pre-screening, outcome classifier (7 verdict types), drift detection, Loop 1/2 gap/clarification integration. Intent hit rate: 55.8% first-attempt / 89.6% two-attempt (from 32.5% / 58.3% baseline). 138 ECIR nouns, 23,405 embeddings, 1,418 verbs
> **Onboarding State View (2026-03-22):** ✅ Complete + Wired — `OnboardingStateView` populated on every `ChatResponse` when group is in scope. 6-layer onboarding DAG (Group Ownership → CBU Identification → KYC Case → Screening → Documents → Approval), per-layer `forward_verbs` + `revert_verbs` (composite-level undo, not factual), per-CBU `CbuStateCard` with `next_action` + `revert_action`, `VerbDirection` enum (Forward/Revert/Query), `suggested_utterance` pipeline-aligned to `HybridVerbSearcher`, `context_reset_hint` for off-context utterances. Computed from live DB composite state (UBO, cases, screenings, docs) via `project_onboarding_state()` in `agent_routes.rs`. 5 unit tests
> **Verb Disambiguation UX (2026-03-22):** ✅ Complete — `VerbOption` enriched with differentiation context: `verb_kind` (primitive/macro/query/workflow), `differentiation` text explaining WHY each option differs, `target_entity_kind`, `constellation_slot`, `entity_context` (human-readable constellation position), `target_entity_name`. Disambiguation message format: each option shows what to say + why it's different + where it operates in the constellation. Clarification round-trip test validates suggested phrases resolve back through the pipeline (79.6% accuracy, all mismatches are synonym-verb pairs within same domain family, zero cross-domain confusion)
...
> **Entity-First Utterance Parsing (2026-03-23):** ✅ Complete — Entity mention extraction now runs BEFORE ECIR noun scanning. `MentionExtractor` identifies entity name spans from in-memory EntitySnapshot (23K+ patterns). Spans passed as exclusion zones to `NounIndex.extract_with_exclusions()` — entity names masked from noun scan. `HybridVerbSearcher.search()` accepts `entity_mention_spans` parameter (8th arg). `LookupService.analyze()` reordered: extract mention spans → mask → verb search. `IntentPipeline` threads entity spans to verb search. Orchestrator stores resolved entities in `session.context.resolved_entities` (UUID cached for subsequent turns). Resolved entity UUID flows through entire pipeline — no double lookup, no `<Entity>` placeholder re-resolution. 19 ECIR unit tests (16 existing + 3 new exclusion tests). Co-resolution algorithm (entity kind ↔ verb subject_kinds mutual boosting) designed, implementation deferred.
> **Pipeline Leak Remediation (2026-03-22):** ✅ Complete — SemOS → Sage → Coder → Execute → SemOS loop closed. All utterance paths flow through SemOS without exception: (1) Phase 5 runtime recheck fetches envelope ONCE before execution loop (eliminated per-statement TOCTOU), (2) pending mutation confirmation rechecks verb against current envelope before executing stale DSL (was bypassing orchestrator), (3) pending states cleared AFTER recheck passes (preserves context on failure), (4) discovery selections validated against SemOS discovery surface before session mutation, (5) dead `phase5_runtime_recheck_agent` removed, (6) `ob-poc-web` borrow-after-move fixed (full workspace clippy clean). Intent hit rate: 64.4% first-attempt, 97.5% two-attempt. Clarification round-trip: 79.6% (zero cross-domain mismatches)
...
| "OnboardingStateView", "onboarding_state", "onboarding DAG", "forward_verbs", "revert_verbs", "CbuStateCard", "VerbDirection", "LayerState", "SuggestedVerb", "context_reset_hint", "project_onboarding_state" | CLAUDE.md §Onboarding State View, `rust/crates/ob-poc-types/src/onboarding_state.rs`, `rust/src/agent/onboarding_state_view.rs` |
...
| "differentiation", "verb_kind", "entity_context", "constellation_slot", "target_entity_kind", "clarification round-trip", "did you mean" | CLAUDE.md §Verb Disambiguation UX, `rust/crates/ob-poc-types/src/disambiguation.rs`, `rust/src/api/agent_service.rs` |
