...
> **Entity-First Utterance Parsing (2026-03-23):** ✅ Complete + Co-Resolution — Entity mention extraction runs BEFORE ECIR noun scanning. `MentionExtractor` identifies entity name spans from in-memory EntitySnapshot. Spans passed as exclusion zones to `NounIndex.extract_with_exclusions()`. Co-resolution: verb `subject_kinds` boost/penalise entity candidates by kind match (+0.15 boost for matching kind, -0.10 penalty for mismatch). "Open a KYC case for HSBC" → kyc-case verbs have subject_kinds=[cbu] → HSBC Custody Services (cbu) boosted over HSBC Holdings (group). Resolved entity UUID cached in `session.context.resolved_entities` for subsequent turns. ECIR accuracy: 86.7% (was 82.4%). Intent hit rate: 65.0% first-attempt, 96.3% two-attempt. 19 ECIR unit tests.
