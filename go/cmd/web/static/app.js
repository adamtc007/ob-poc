...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const cbuSelect = document.getElementById("cbu-select");
  const refreshCbusBtn = document.getElementById("refresh-cbus-btn");
  ...
  // ==================== CBU PICKER ====================
  // Fetch list of CBUs for the picker dropdown
  async function loadCbuList() {
    try {
      const resp = await fetch("/api/cbus");
      if (!resp.ok) {
        console.error("Failed to load CBUs:", resp.status);
        return;
      }
      const cbus = await resp.json();
      // Clear existing options (except the placeholder)
      while (cbuSelect.options.length > 1) {
        cbuSelect.remove(1);
      }
      // Add CBU options
      for (const cbu of cbus) {
        ...
        option.textContent = `${cbu.name}${jurisdiction}`;
        option.dataset.name = cbu.name;
        option.dataset.jurisdiction = cbu.jurisdiction || "";
        option.dataset.clientType = cbu.client_type || "";
        cbuSelect.appendChild(option);
      }
    } catch (e) {
      console.error("Error loading CBU list:", e);
    }
  }
  // Handle CBU selection from dropdown
  async function handleCbuSelect() {
    const cbuId = cbuSelect.value;
    if (!cbuId) {
      // Cleared selection - reset context
      context.cbu = null;
      debugState.cbu = null;
      updateDebugPanels();
      updateContextBar();
      cbuSelect.classList.remove("active-cbu");
      return;
    }
    // Get metadata from selected option
    const selectedOption = cbuSelect.options[cbuSelect.selectedIndex];
    const cbuName = selectedOption.dataset.name;
    const jurisdiction = selectedOption.dataset.jurisdiction;
    const clientType = selectedOption.dataset.clientType;
    // Set as active CBU in context
    context.cbu = {
      id: cbuId,
      name: cbuName,
    ...
    // Store in bindings for DSL generation
    context.bindings.set("cbu", cbuId);
    // Visual indicator that a CBU is active
    cbuSelect.classList.add("active-cbu");
    // Fetch full CBU data and update panels
    await fetchAndDisplayCbu(cbuId);
    // Add system message to chat
    clearEmptyState();
    addAssistantMessage({
      text: `Switched to CBU: "${cbuName}"${jurisdiction ? ` (${jurisdiction})` : ""}. You can now run incremental operations on this CBU.`,
      status: {
        type: "valid",
        text: `Active: @cbu = ${cbuId.substring(0, 8)}...`,
      },
    });
    updateContextBar();
  }
  ...
  async function updateContextFromBindings(bindings) {
    ...
    if (cbuId && !context.cbu) {
      await fetchAndDisplayCbu(cbuId);
      // Update the CBU picker to show the new CBU
      await loadCbuList();
      cbuSelect.value = cbuId;
      cbuSelect.classList.add("active-cbu");
    }
  ...
  async function executeDsl(dsl) {
    ...
    try {
      // Auto-inject active CBU ID if DSL needs it (except for cbu.create/ensure)
      let finalDsl = dsl;
      const isCbuCreate = /\(cbu\.(ensure|create)\s/.test(dsl);

      if (!isCbuCreate && context.cbu && context.cbu.id) {
        // Replace @cbu references with actual UUID
        finalDsl = finalDsl.replace(/@cbu\b/g, `"${context.cbu.id}"`);
      }

      const resp = await fetch("/api/dsl/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dsl: finalDsl }),
      });
      const data = await resp.json();

      if (data.success) {
        // Server returns primary_keys array with entity_type, subtype, id, name
        // Each verb that produces an entity returns its primary key
        if (data.primary_keys && data.primary_keys.length > 0) {
          for (const pk of data.primary_keys) {
            // Store binding by entity_type (e.g., "cbu", "entity")
            context.bindings.set(pk.entity_type, pk.id);

            // If this is a CBU, set it as the active CBU
            if (pk.entity_type === "cbu") {
              context.cbu = {
                id: pk.id,
                name: pk.name || "CBU",
                jurisdiction: null,
              ...
              // Update CBU picker
              await loadCbuList();
              cbuSelect.value = pk.id;
              cbuSelect.classList.add("active-cbu");
              // Fetch full CBU data for display
              await fetchAndDisplayCbu(pk.id);
            }

            // If this is an entity, store it in entities map
            if (pk.entity_type === "entity") {
              const bindingName = pk.name
                ? pk.name
                    .toLowerCase()
                    .replace(/\s+/g, "_")
                    .replace(/[^a-z0-9_]/g, "")
        ...
        // Refresh CBU structure after successful execution
        if (context.cbu && context.cbu.id) {
          await fetchCbuData(context.cbu.id);
        }
      } else {
        const errorMsg =
  ...
  function getContextForAgent() {
    const parts = [];
    if (context.cbu) {
      // Include the actual CBU UUID so agent can use it directly
      parts.push(`ACTIVE_CBU: "${context.cbu.name}" (id: ${context.cbu.id})`);
      parts.push(
        `For any :cbu-id parameter, use the UUID: "${context.cbu.id}"`,
      );
    }
    if (context.case) {
      parts.push(
  ...
  // CBU picker event listeners
  cbuSelect.addEventListener("change", handleCbuSelect);
  refreshCbusBtn.addEventListener("click", loadCbuList);
  // Initialize panels and load CBU list
  initPanels();
  loadCbuList();
});
