...
document.addEventListener("DOMContentLoaded", () => {
     ...
     */
    async function fetchCbuGraph(cbuId) {
        try {
            const resp = await fetch(`/api/cbu/${cbuId}/graph?view_mode=${graphState.viewMode}&orientation=${graphState.orientation}`);
            if (!resp.ok) {
    ...
    async function handleViewModeChange(newMode) {
        if (newMode === graphState.viewMode)
            return;
    ...
    async function handleOrientationChange(newOrientation) {
        if (newOrientation === graphState.orientation)
            return;
