...
fn build_fixture_events(subject: SubjectId) -> Vec<IntentEvent> {
    ...
    vec![
        ...
        // Economic edges.
        // B → A: 60%
        te(5, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({
                "edge_id": eid("b_a").0,
                "from_entity_id": b.0,
        ...
        // P1 → B: 80%
        te(6, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({
                "edge_id": eid("p1_b").0,
                "from_entity_id": p1.0,
        ...
        // P2 → B: 20%
        te(7, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({
                "edge_id": eid("p2_b").0,
                "from_entity_id": p2.0,
        ...
        // P3 → A: 40%
        te(8, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({
                "edge_id": eid("p3_a").0,
                "from_entity_id": p3.0,
        ...
        // Reconcile conflict.
        te(9, subject, "ubo.edge.reconcile-conflict", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"note": "single-source; no conflict"}),
            idem("reconcile"), t,
        ),
        // Select strategy.
        te(10, subject, "ubo.determination.select-strategy", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"strategy": "ownership_prong_strategy"}),
            idem("select-strategy"), t,
        ),
    ]
}
...
fn fixture_natural_persons() -> BTreeSet<PersonId> {
    [person_p1(), person_p2(), person_p3()].into_iter().collect()
}
...
fn ec1_ownership_prong_differential_equality() {
    ...
    assert_eq!(candidates.len(), 2, "expected P1 and P3 only; got: {:?}", candidates);

    let p1_cand = candidates.iter().find(|c| c.person_id == person_p1())
        .expect("P1 must be a candidate");
    let p3_cand = candidates.iter().find(|c| c.person_id == person_p3())
    ...
    for c in &candidates {
        assert_eq!(c.prong, Prong::OwnershipProng, "prong must be OwnershipProng (K-1)");
    }
}
...
fn ec2_conflicting_edges_fail_without_reconcile() {
    ...
    let events = vec![
        ...
        te(2, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"edge_id": eid("src1_a").0,
                "from_entity_id": Uuid::new_v4(), "to_entity_id": a.0, "percentage": 70.0}),
            idem("e1"), t,
        ),
        te(3, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
    ...
    assert!(total > 100.0, "total claimed % should exceed 100: got {total}");

    // Check precondition for compute-fold: ReconciledProjection must fail.
    let compute_entry = lexicon.get("ubo.determination.compute-fold")
        .expect("compute-fold in lexicon");
    let dummy_event = te(
        4, subject, "ubo.determination.compute-fold", h, analyst(), authority(),
        TargetBinding::for_subject(subject),
        serde_json::json!({}),
        idem("fold"), t,
    );
...
fn ec3_verify_without_evidence_is_rejected() {
    ...
    let edge = eid("b_a");
    let events = vec![
        te(0, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
    ...
    let verify_event = te(
        1, subject, "ubo.edge.verify", h, analyst(), authority(),
        TargetBinding::for_edge(subject, edge),
        serde_json::json!({}),
        idem("verify-attempt"), t,
    );
...
fn ec3_verify_after_evidence_succeeds() {
    ...
    let events = vec![
        ...
        // Attach evidence.
        te(1, subject, "ubo.edge.attach-evidence", h, analyst(), authority(),
            TargetBinding::for_edge(subject, edge),
    ...
    let edge_state = control.edges.get(&edge).expect("edge must exist");
    assert_eq!(edge_state.status, ob_poc_kyc_substrate::EdgeStatus::Evidenced);

    // Verify must now pass the precondition.
    let verify_entry = lexicon.get("ubo.edge.verify").expect("verify in lexicon");
    let verify_event = te(
        2, subject, "ubo.edge.verify", h, analyst(), authority(),
        TargetBinding::for_edge(subject, edge),
        serde_json::json!({}),
        idem("verify"), t,
    );
    let result = check_control_preconditions(verify_entry, &control, &verify_event);
    assert!(result.is_ok(), "verify after evidence must pass: {:?}", result);
...
fn ec4_smo_fallback_when_no_ubos_found() {
    ...
    let events = vec![
        ...
        te(2, subject, "ubo.edge.reconcile-conflict", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"note": "empty graph; no conflict"}),
            idem("reconcile"), t,
        ),
        te(3, subject, "ubo.determination.select-strategy", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"strategy": "ownership_prong_strategy"}),
            idem("strategy"), t,
        ),
        // SMO fallback applied.
        te(4, subject, "ubo.determination.apply-smo-fallback", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"smo_person_id": person_smo().0}),
            idem("smo"), t,
        ),
        // Freeze.
        te(5, subject, "ubo.determination.freeze", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({}),
            idem("freeze"), t,
        ),
    ...
    assert_eq!(control.smo_person_id, Some(person_smo()), "SMO person must be set in control state");

    // Build determination-in-progress.
    let edges = reconciled_economic_edges(&control);
    let strategy = OwnershipProngStrategy;
    let candidates = strategy.resolve(&edges, entity_subject(), &BTreeSet::new(), 25.0);
    assert!(candidates.is_empty(), "no candidates expected for empty graph");
    ...
    let smo_orig_event_id = control.smo_event_id
    ...
    assert!(frozen.candidates.is_empty(), "no ownership candidates expected");

    // Phase 2 gate: assert SMO provenance, not just presence (K-5, K-35).
    let smo = frozen.smo_result.as_ref().expect("SMO result must be present (K-5)");
    match smo {
        SmoResult::Person(c) => {
            assert_eq!(c.prong, Prong::SmoFallback, "SMO candidate must carry SmoFallback prong");
            assert_ne!(
...
fn ec4_freeze_without_candidates_or_smo_fails() {
    ...
    let events = vec![
        ...
        te(1, subject, "ubo.edge.reconcile-conflict", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({}), idem("reconcile"), t,
        ),
        te(2, subject, "ubo.determination.select-strategy", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"strategy": "ownership_prong_strategy"}),
            idem("strategy"), t,
        ),
        te(3, subject, "ubo.determination.freeze", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({}), idem("freeze"), t,
        ),
    ];
    let event_refs: Vec<&IntentEvent> = events.iter().collect();
    let control = fold_control(&event_refs);
    ...
    let freeze_event = events.last().unwrap();
    let result = freeze_determination(
        &det, &control, freeze_event,
        "v1.0", dummy_hash(), Uuid::new_v4(), BTreeSet::new(),
    );
    assert!(result.is_err(), "freeze with no candidates and no SMO must fail (K-5)");
}
...
fn ec5_replay_determinism_after_supersede() {
    ...
    let mut events: Vec<IntentEvent> = vec![
        ...
        // P1 → B: 100%
        te(4, subject, "ubo.edge.assert-economic-interest", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"edge_id": eid("p1_b").0,
                "from_entity_id": p1_entity.0, "to_entity_id": b.0, "percentage": 100.0}),
            idem("edge-p1-b"), t1,
        ),
        te(5, subject, "ubo.edge.reconcile-conflict", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({}), idem("reconcile1"), t1,
        ),
        te(6, subject, "ubo.determination.select-strategy", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"strategy": "ownership_prong_strategy"}),
            idem("strategy"), t1,
        ),
        // First freeze at seq=7 (t1).
        te(7, subject, "ubo.determination.freeze", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({}), idem("freeze1"), t1,
        ),
    ];
    ...
    assert_eq!(prior_det.candidates.len(), 1, "prior determination should have P1");
    ...
    events.push(
        te(8, subject, "ubo.edge.supersede", h, analyst(), authority(),
            TargetBinding::for_edge(subject, edge_b_a),
            serde_json::json!({"reason": "company sold"}),
            idem("supersede-b-a"), t2,
        ),
    );
    events.push(
        te(9, subject, "ubo.edge.reconcile-conflict", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({}), idem("reconcile2"), t2,
        ),
    );
    // No ownership UBOs after restructuring → SMO fallback required (K-5).
    events.push(
        te(10, subject, "ubo.determination.apply-smo-fallback", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({"smo_person_id": person_smo().0}),
            idem("smo2"), t2,
        ),
    ...
    events.push(
        te(11, subject, "ubo.determination.freeze", h, analyst(), authority(),
            TargetBinding::for_subject(subject),
            serde_json::json!({}), idem("freeze2"), t2,
        ),
    );
    ...
    let latest_smo = latest_det.smo_result.as_ref()
        .expect("post-supersede: SMO result must be present (K-5)");
    match latest_smo {
        SmoResult::Person(c) => {
            assert_eq!(c.prong, Prong::SmoFallback,
                "post-supersede SMO must carry SmoFallback prong");
            assert_ne!(
                c.originating_event_id.0, EventId::default().0,
    ...
    assert_eq!(
        prior_det.determination_hash,
        replay_det.determination_hash,
...
fn ec7_multi_role_person_one_subject_two_obligations() {
    ...
    let obl_shareholder = ObligationId(Uuid::parse_str("50000000-0000-0000-0000-000000000001").unwrap());
    let obl_director    = ObligationId(Uuid::parse_str("50000000-0000-0000-0000-000000000002").unwrap());
    ...
    let events = vec![
        ...
        te(0, subject, "kyc.subject.register", h, analyst(), authority(),
            TargetBinding { subject_root: Some(person_subject), ..Default::default() },
            serde_json::json!({"entity_id": person.0, "is_natural_person": true, "role": "shareholder"}),
            idem("reg-p1"), t,
        ),
        // Obligation 1: shareholder basis.
        te(1, subject, "kyc.obligation.create", h, analyst(), authority(),
            TargetBinding { subject_root: Some(person_subject), ..Default::default() },
            serde_json::json!({
            ...
            idem("obl-shareholder"), t,
        ),
        // Obligation 2: director basis.
        te(2, subject, "kyc.obligation.create", h, analyst(), authority(),
            TargetBinding { subject_root: Some(person_subject), ..Default::default() },
            serde_json::json!({
                "subject_id": person_subject.0,
    ...
    assert_eq!(
        obl_state.subjects.len(), 1,
        "one subject entry expected; got {}", obl_state.subjects.len(),
    );
    let rollup = obl_state.subjects.get(&person_subject)
        .expect("person_subject must have a rollup");

    // TWO distinct obligations on that subject (K-21: each has its own basis).
    assert_eq!(
        rollup.obligations.len(), 2,
        "two obligations expected; got {}", rollup.obligations.len(),
    );

    let obl1 = obl_state.obligations.get(&obl_shareholder)
        .expect("shareholder obligation must exist");
    let obl2 = obl_state.obligations.get(&obl_director)
        .expect("director obligation must exist");

    // Distinct bases (K-21).
    assert_ne!(obl1.basis.role, obl2.basis.role, "obligations must have distinct bases");
    ...
    assert_ne!(
        obl1.originating_event_id.0, EventId::default().0,
        "shareholder obligation missing originating_event_id",
    );
    assert_ne!(
        obl2.originating_event_id.0, EventId::default().0,
...
fn k13_superseded_edges_remain_in_fold() {
    ...
    let events = vec![
        ...
        te(1, subject, "ubo.edge.supersede", h, analyst(), authority(),
            TargetBinding::for_edge(subject, edge),
    ...
    let edge_state = control.edges.get(&edge)
...
fn k30_all_phase1_verbs_declare_governing_taxonomy() {
    ...
    for (fqn, entry) in &lexicon.entries {
        ...
        assert!(!entry.writes.is_empty(), "verb {fqn} must declare at least one writes-fold");
        assert!(!entry.fqn.0.is_empty(), "verb {fqn} must have a non-empty FQN");
    }
}
...
fn phase1_fold_determinism_stress_graph_hash() {
    ...
    let hashes: Vec<ob_poc_kyc_substrate::Hash> = (0..10).map(|_| {
        let events = build_fixture_events(subject);
        let event_refs: Vec<&IntentEvent> = events.iter().collect();
        let control = fold_control(&event_refs);
        let edges = reconciled_economic_edges(&control);
...
fn phase1_fold_determinism_stress_determination_hash() {
    ...
    let t = ts(2026, 1, 1);
    base_events.push(te(
        11, subject, "ubo.determination.freeze", h, analyst(), authority(),
        TargetBinding::for_subject(subject),
        serde_json::json!({}), idem("freeze-stress"), t,
    ));
    ...
    let hashes: Vec<ob_poc_kyc_substrate::Hash> = (0..10).map(|_| {
        ...
        ).expect("determination must succeed").determination_hash
    }).collect();
