...
async fn linear_sequence_roundtrips() {
    roundtrip(
        include_str!("../../dsl-migrate/tests/corpus/linear_sequence.bpmn"),
        "linear-sequence",
    )
    .await;
}
...
async fn exclusive_gateway_roundtrips() {
    roundtrip(
        include_str!("../../dsl-migrate/tests/corpus/exclusive_gateway.bpmn"),
        "exclusive-gateway",
    )
    .await;
}
...
async fn parallel_fork_join_roundtrips() {
    roundtrip(
        include_str!("../../dsl-migrate/tests/corpus/parallel_fork_join.bpmn"),
        "parallel-fork-join",
    )
    .await;
}
...
async fn boundary_events_roundtrip() {
    roundtrip(
        include_str!("../../dsl-migrate/tests/corpus/boundary_events.bpmn"),
        "boundary-events",
    )
    .await;
}
...
async fn feel_expressions_roundtrip() {
    ...
    roundtrip(
        include_str!("../../dsl-migrate/tests/corpus/feel_expressions.bpmn"),
        "feel-expressions",
    )
    .await;
}
...
async fn feel_conditions_complex_roundtrip() {
    ...
    roundtrip(
        include_str!("../../dsl-migrate/tests/corpus/feel_conditions_complex.bpmn"),
        "feel-complex",
    )
    .await;
}
...
async fn user_task_with_form_roundtrips() {
    ...
    roundtrip(
        include_str!("../../dsl-migrate/tests/corpus/user_task_with_form.bpmn"),
        "user-task-form",
    )
    .await;
}
