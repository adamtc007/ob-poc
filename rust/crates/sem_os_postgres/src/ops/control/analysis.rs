...
pub struct ControlTraceChain;

#[async_trait]
impl SemOsVerbOp for ControlTraceChain {
    fn fqn(&self) -> &str {
        "control.trace-chain"
    }
    async fn execute(
        ...
        let result = if let Some((path, depth)) = chain {
            let mut chain_details: Vec<Value> = Vec::new();
...
pub struct ControlReconcileOwnership;

#[async_trait]
impl SemOsVerbOp for ControlReconcileOwnership {
    fn fqn(&self) -> &str {
        "control.reconcile-ownership"
    }
    async fn execute(
        ...
        let share_ownership: Vec<ShareOwnershipRow> = sqlx::query_as(
            ...
            WITH share_totals AS (
                SELECT h.investor_entity_id, SUM(h.units) as total_units
