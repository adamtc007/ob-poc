...
import { EntityFinderModal, } from "./entity-finder.js";
class App {
    constructor() {
    ...
    async bindCbuToSession(cbuId) {
        const sessionId = this.chatPanel.getSessionId();
        if (!sessionId) {
            console.warn("[App] No session to bind CBU to");
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log("[App] CBU bound to session:", result.binding_name, "=", cbuId);
            }
            else {
            ...
            console.error("[App] Error binding CBU to session:", error);
        }
    }
    ...
    async handleEntityResolution(ctx, match) {
        ...
        try {
            ...
            console.log("[App] Reference resolved. Remaining:", result.resolution_stats?.unresolved_count);
        }
        catch (error) {
