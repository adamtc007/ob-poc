...
export class AstPanel {
    ...
    setupEventListeners() {
        ...
        this.treeEl.addEventListener("click", (e) => {
            ...
            const nodeEl = target.closest(".ast-node");
            if (nodeEl?.classList.contains("unresolved") &&
                ...
                const argKey = argEl?.querySelector(".ast-keyword")?.textContent?.replace(":", "") ||
                    "";
    ...
    renderValue(value, nodeId) {
        ...
        if (typeof value === "object") {
            const entries = Object.entries(value).filter(([k]) => k !== "Literal" && k !== "SymbolRef" && k !== "EntityRef");
            const items = entries
                .map(([k, v], idx) => `
