...
export class DslPanel {
    ...
    private highlight(dsl: string): string {
    ...
    private highlightLine_(line: string): string {
        // Comment
        ...
        result = result.replace(
            /\b([\w-]+)\.([\w-]+)\b/g,
            '<span class="dsl-verb">$1.$2</span>'
        );

        // Strings ("...")
        result = result.replace(
            /"([^"\\]|\\.)*"/g,
            '<span class="dsl-string">$&</span>'
        );

        // Symbols (@name)
        result = result.replace(
            /@[\w_]+/g,
            '<span class="dsl-symbol">$&</span>'
        );

        // Brackets
        result = result.replace(
            /[()[\]{}]/g,
            '<span class="dsl-bracket">$&</span>'
        );
    ...
    private updateStatus(status: 'empty' | 'pending' | 'executed' | 'error') {
