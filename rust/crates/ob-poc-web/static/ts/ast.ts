...
export interface UnresolvedRefContext {
...
export type AstCallback = {
  ...
  onUnresolvedRefClick?: (context: UnresolvedRefContext) => void;
};
...
export class AstPanel {
  ...
  constructor(callbacks: AstCallback) {
    ...
    this.expandAllBtn = document.getElementById(
      "ast-expand-all",
    ) as HTMLButtonElement;
    this.collapseAllBtn = document.getElementById(
      "ast-collapse-all",
    ) as HTMLButtonElement;
  ...
  private setupEventListeners() {
    ...
    this.treeEl.addEventListener("click", (e) => {
      ...
      // Check for click on unresolved EntityRef
      const nodeEl = target.closest(".ast-node") as HTMLElement;
      if (
        nodeEl?.classList.contains("unresolved") &&
        nodeEl.dataset.type === "EntityRef"
      ) {
        const entityType = nodeEl.dataset.entityType || "entity";
        const searchText = nodeEl.dataset.searchText || "";
        const nodeId = nodeEl.dataset.id || "";

        // Parse nodeId to extract statement index
        // Format: "0-arg-1-val" where 0 is statement index
        const parts = nodeId.split("-");
        const statementIndex = parseInt(parts[0], 10) || 0;
        const argEl = nodeEl.closest('[data-type="Argument"]');
        const argKey =
          argEl?.querySelector(".ast-keyword")?.textContent?.replace(":", "") ||
          "";

        if (this.callbacks.onUnresolvedRefClick) {
          this.callbacks.onUnresolvedRefClick({
            statementIndex,
            argKey,
            entityType,
            searchText,
  ...
  setAst(statements: AstStatement[]) {
    if (!statements || statements.length === 0) {
      this.treeEl.innerHTML =
        '<span style="color: var(--text-muted)">AST will appear here</span>';
      return;
    }
  ...
  private renderVerbCall(vc: VerbCallData, nodeId: number): string {
    ...
    const bindingStr = vc.binding
      ? ` <span class="ast-symbol">@${vc.binding}</span>`
      : "";

    const argsHtml = vc.arguments
      .map((arg, idx) => this.renderArgument(arg, `${nodeId}-arg-${idx}`))
      .join("");
  ...
  private renderValue(value: AstValue, nodeId: string): string {
    ...
    if (this.isEntityRef(value)) {
      ...
      const unresolvedClass = ref.resolved_key ? "" : "unresolved clickable";
      const resolvedStr = ref.resolved_key
        ? `<span class="ast-resolved">→ ${ref.resolved_key.substring(0, 8)}...</span>`
        : '<span class="ast-unresolved">(click to resolve)</span>';

      return `
                <div class="ast-node ${unresolvedClass}" data-id="${nodeId}" data-type="EntityRef" data-entity-type="${ref.entity_type}" data-search-text="${this.escapeHtml(ref.value)}">
                    <span class="ast-toggle"> </span>
    ...
    if (Array.isArray(value)) {
      const items = value
        .map((v, idx) => this.renderValue(v, `${nodeId}-${idx}`))
        .join("");
      return `
    ...
    if (typeof value === "object") {
      const entries = Object.entries(value).filter(
        ([k]) => k !== "Literal" && k !== "SymbolRef" && k !== "EntityRef",
      );
      const items = entries
        .map(
          ([k, v], idx) => `
  ...
  private hasChildren(value: AstValue): boolean {
    ...
    if (
      ...
      this.isSymbolRef(value) ||
      this.isEntityRef(value)
    )
      return false;
