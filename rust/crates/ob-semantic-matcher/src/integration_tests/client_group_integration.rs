...
mod tests {
    ...
    static SHARED_EMBEDDER: OnceCell<Arc<Embedder>> = OnceCell::const_new();
    ...
    pub(crate) async fn get_embedder() -> &'static Arc<Embedder> {
        SHARED_EMBEDDER
            .get_or_init(|| async {
                let embedder = tokio::task::spawn_blocking(|| {
                    Embedder::new().expect("Failed to load embedder")
                })
                .await
    ...
    /// Wrapper to make Embedder implement the crate::client_group_resolver::Embedder trait
    pub(crate) struct EmbedderAdapter(pub(crate) Arc<Embedder>);

    #[async_trait::async_trait]
    impl crate::client_group_resolver::Embedder for EmbedderAdapter {
        async fn embed_query(&self, text: &str) -> Result<Vec<f32>, String> {
            let text_owned = text.to_string();
            let embedder = self.0.clone();
            tokio::task::spawn_blocking(move || embedder.embed_query(&text_owned))
                .await
        ...
        async fn embed_target(&self, text: &str) -> Result<Vec<f32>, String> {
            let text_owned = text.to_string();
            let embedder = self.0.clone();
            tokio::task::spawn_blocking(move || embedder.embed_target(&text_owned))
                .await
                .map_err(|e| e.to_string())?
                .map_err(|e| e.to_string())
        }
