//! Step Result Types
//!
//! `StepResult` provides detailed results for individual execution steps.
//! `ExecutionResults` accumulates results across an entire execution plan.
...
///
/// This provides more detailed information than `ExecutionResult`, capturing
/// the specific operation type and any metadata about what happened.
...
impl StepResult {
    ...
    pub fn is_delete(&self) -> bool {
        matches!(self, StepResult::Deleted { .. })
    }
    ...
    pub fn type_name(&self) -> &'static str {
        match self {
            StepResult::Created { .. } => "created",
            StepResult::Updated { .. } => "updated",
            StepResult::Deleted { .. } => "deleted",
            StepResult::Read { .. } => "read",
            StepResult::List { .. } => "list",
            StepResult::Linked { .. } => "linked",
            StepResult::NoOp => "noop",
            StepResult::Custom { .. } => "custom",
...
impl ExecutionResults {
    /// Create a new empty results accumulator
    pub fn new() -> Self {
        Self::default()
    }
    ...
    pub fn steps(&self) -> &[(usize, StepResult)] {
        &self.step_results
    }
...
mod tests {
    ...
    fn test_step_result_entity_type() {
        ...
        assert_eq!(created.entity_type(), Some("cbu"));

        let noop = StepResult::NoOp;
        assert_eq!(noop.entity_type(), None);
    }
    ...
    fn test_step_result_type_name() {
        ...
        assert_eq!(StepResult::NoOp.type_name(), "noop");
        assert_eq!(
            StepResult::Skipped {
    ...
    #[test]
    fn test_execution_results_summary() {
        let mut results = ExecutionResults::new();
        results.record_step(
            0,
            StepResult::Created {
        ...
        assert!(summary.contains("1 creates"));
        assert!(summary.contains("1 updates"));
        assert!(summary.contains("1 bindings"));
