...
fn register_event(subject: SubjectId, idem: &str) -> IntentEvent {
    IntentEvent::new(
        subject, "kyc.subject.register", Principal::test_analyst(),
        AuthorityRef("analyst".into()), TargetBinding::for_subject(subject),
...
async fn cleanup(pool: &PgPool, subjects: &[SubjectId]) {
    for s in subjects {
        ...
        let _ = sqlx::query(r#"DELETE FROM "public".outbox WHERE idempotency_key LIKE $1"#)
            .bind(format!("{}:%", s.0)).execute(pool).await;
...
async fn exit6_replay_no_redispatch() {
    ...
    PgKycEventStore::append(&mut tx, &registry, &register_event(subject, "reg"), |_| Ok(()))
        .await.unwrap();
    ...
    assert!(pending_after_append > 0, "append must enqueue at least one projection effect");
    ...
    assert_eq!(before, after, "exit 6: replay (load+fold) enqueues zero outbox effects");
...
async fn exit9_cross_stream_b2_retraction_and_b3_dedupe() {
    ...
    let r1 = enqueue_cross_stream_obligations(
        &mut conn, freeze_event_id, determination_subject,
        &[person_a, person_b], &[], "ubo_candidate", correlation,
    ).await.unwrap();
    ...
    let r_b3 = enqueue_cross_stream_obligations(
        &mut conn, freeze_event_id2, determination_subject,
        &[person_a, person_b], &[], "ubo_candidate", correlation,
    ).await.unwrap();
    ...
    let r2 = enqueue_cross_stream_obligations(
        &mut conn, freeze_event_id3, determination_subject,
        &[person_a], // only A now
        &prior,
        "ubo_candidate", correlation,
    ).await.unwrap();
    assert_eq!(r2.creates, 0, "B2: A already in prior, no new create");
    assert_eq!(r2.supersedes, 1, "B2: B dropped → obligation-supersede emitted");
