...
mod tests {
    ...
    fn test_basic_workflow_parsing() {
        ...
        (workflow "test-workflow"
            (properties
                (nature-purpose "Test workflow")
            )
            (declare-entity "entity1" COMPANY
                (properties
                    (legal-name "Test Company")
                )
            )
        )
        "#;

        let result = parser.parse_with_semantics(input);
        assert!(result.is_ok());

        let parse_result = result.unwrap();
        assert_eq!(parse_result.program.workflows.len(), 1);
        assert_eq!(parse_result.program.workflows[0].id, "test-workflow");
    }
    ...
    fn test_type_inference() {
        ...
        let inferred_type = inferrer.infer_value_type(&string_value);

        match inferred_type {
            DSLType::String { .. } => assert!(true),
            _ => panic!("Expected String type"),
        }

        let number_value = Value::Number(42.0);
        let number_type = inferrer.infer_value_type(&number_value);

        match number_type {
            DSLType::Number { .. } => assert!(true),
    ...
    fn test_semantic_validation() {
        ...
        let semantic_info = vec![];
        let type_info = TypeAnalysisResult {
            type_mappings: HashMap::new(),
            constraint_violations: vec![],
            inference_results: vec![],
        };

        let result = validator.validate(&program, &semantic_info, &type_info);
        assert!(result.is_ok());
    ...
    #[test]
    fn test_empty_workflow_id_validation() {
        let mut validator = SemanticValidator::new();
        let program = Program {
            workflows: vec![Workflow {
                id: "".to_string(), // Empty ID should cause validation error
                properties: HashMap::new(),
                statements: vec![],
            }],
        };
        ...
        assert_eq!(validation_results.errors[0].code, "EMPTY_WORKFLOW_ID");
    }
}
