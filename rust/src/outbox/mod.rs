...
// `pub` (not `pub(crate)`/cfg(test)): the trait lives in the private
// `consumer` module but is implemented across the crate boundary by the
// `ob-poc-web` binary crate's consumers (and by `integration_tests`).
