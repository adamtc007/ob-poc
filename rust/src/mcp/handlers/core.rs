...
impl ToolHandlers {
    ...
    async fn dispatch(&self, name: &str, args: Value) -> Result<Value> {
        match name {
            ...
            "esper_list" => self.esper_list(args).await,
        ...
        let commands: Vec<serde_json::Value> = config
            ...
            .map(|(key, def)| {
                json!({
                    ...
                    "aliases": {
                        "exact": def.aliases.exact,
                        "contains": def.aliases.contains,
                        "prefix": def.aliases.prefix
                    }
                })
            })
            .collect();
    ...
    /// Test what ESPER command a phrase maps to
    async fn esper_lookup(&self, args: Value) -> Result<Value> {
        use crate::agent::esper::{EsperWarmup, MatchSource};
        ...
        match registry.lookup(&args.phrase) {
            Some(m) => Ok(json!({
                ...
                "command": format!("{:?}", m.command),
                "response": m.response,
                "source": match m.source {
                    MatchSource::Builtin => "Builtin",
                    MatchSource::Learned => "Learned",
                },
                "extracted_params": m.extracted_params
            })),
            None => Ok(json!({
                "matched": false,
                "message": "No ESPER command matched. Will fall through to DSL pipeline."
            })),
        }
    }

    /// Add a learned alias for an ESPER command
    async fn esper_add_alias(&self, args: Value) -> Result<Value> {
        use crate::agent::esper::EsperWarmup;
        ...
        if !config.commands.contains_key(&args.command_key) {
            return Err(anyhow!(
                "Unknown command_key: {}. Use esper_list to see available commands.",
                args.command_key
            ));
        }

        // Persist to database using upsert function
        sqlx::query(r#"SELECT agent.upsert_esper_alias($1, $2)"#)
            .bind(args.phrase.to_lowercase().trim())
            .bind(&args.command_key)
            .execute(pool)
            .await?;

        Ok(json!({
            "success": true,
            "phrase": args.phrase.to_lowercase().trim(),
            "command_key": args.command_key,
            "message": "Alias added. It will take effect on next server restart or esper_reload."
        }))
    }

    /// Reload ESPER configuration and learned aliases
    async fn esper_reload(&self, _args: Value) -> Result<Value> {
        ...
        Ok(json!({
            "success": true,
            "message": "ESPER configuration reloaded",
            "stats": {
