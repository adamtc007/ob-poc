...
// T11.1b (2026-07-12): IntentArgValue/IntentArgument/StructuredIntent hoisted
// to ob-poc-types::intent — needed by ob-poc-agent's Sage arg-assembly/
// drafting engines (now living outside this crate), which cannot depend on
// ob-poc (L1). Re-exported here so every existing `crate::mcp::
// intent_pipeline::{IntentArgValue, IntentArgument, StructuredIntent}`
// caller continues to resolve unchanged.
pub use ob_poc_types::intent::{IntentArgValue, IntentArgument, StructuredIntent};
...
// T11.1b (2026-07-12): assemble_dsl_string/format_intent_value_string_only
// hoisted to ob-poc-types::intent alongside StructuredIntent/IntentArgValue
// — needed by ob-poc-agent's Sage arg-assembly/drafting engines (now
// living outside this crate), which cannot depend on ob-poc (L1).
// Re-exported here so every existing `crate::mcp::intent_pipeline::
// assemble_dsl_string` caller continues to resolve unchanged.
pub use ob_poc_types::intent::{assemble_dsl_string, format_intent_value_string_only};
