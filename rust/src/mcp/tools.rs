...
- Hit rate metrics"#.into(),
        ...
        // =========================================================================
        // ESPER NAVIGATION TOOLS
        // =========================================================================
        Tool {
            name: "esper_list".into(),
            description: r#"List all available ESPER navigation commands.

Returns all 48 commands with:
- command_key: Internal identifier (e.g., "zoom_in", "scale_universe")
- canonical: The "proper" phrase to say (e.g., "enhance")
- aliases: All recognized phrases (exact, contains, prefix)
- response: What the system says back
- priority: Match priority (higher wins for overlaps)

Optionally filter by category to see related commands."#.into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "category": {
                        "type": "string",
                        "enum": ["stop", "zoom", "pan", "scale", "depth", "orbital", "temporal", "investigation", "context", "layout", "export", "search", "help"],
        ...
        Tool {
            name: "esper_lookup".into(),
            description: r#"Test what ESPER command a phrase maps to.

Use this to check if a user phrase will be recognized as a navigation
command, or if it will fall through to the DSL pipeline.

Returns:
- matched: true/false
- command_key: Internal identifier (if matched)
- command: The AgentCommand variant
- response: What would be shown to user
- source: "Builtin" or "Learned"
- extracted_params: Any extracted parameters (factor, direction, etc.)"#.into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "phrase": {
                        "type": "string",
                        "description": "The phrase to test (e.g., 'enhance 2x', 'show universe')"
                    }
                },
                "required": ["phrase"]
            }),
        ...
        Tool {
            name: "esper_add_alias".into(),
            description: r#"Add a learned alias for an ESPER command.

Maps a new phrase to an existing command. The alias will be:
1. Added to the in-memory trie immediately
2. Persisted to the database (survives server restart)

Use this when users consistently say something that should map to
an existing command but doesn't. Example: "make it bigger" → zoom_in"#.into(),
            input_schema: json!({
                "type": "object",
                "properties": {
                    "phrase": {
                        "type": "string",
                        "description": "New phrase to learn (e.g., 'make it bigger')"
                    },
                    "command_key": {
                        "type": "string",
                        "description": "Target command key (e.g., 'zoom_in', 'scale_universe')"
                    }
                },
                "required": ["phrase", "command_key"]
            }),
        },
        Tool {
            name: "esper_reload".into(),
            description: r#"Reload ESPER configuration and learned aliases.

Reloads from:
1. YAML config file (rust/config/esper-commands.yaml)
2. Learned aliases from database (agent.esper_aliases)
