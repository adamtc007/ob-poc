...
pub struct DualDecodeResult {
    /// Top verb match after joint scoring.
    pub verb: String,
    /// Score after pack scoring.
    pub score: f32,
    /// Pack that best explains this verb (if any).
    pub pack_id: Option<String>,
    /// Ambiguity outcome.
    pub outcome: AmbiguityOutcome,
    /// All scored candidates (for DecisionLog).
...
/// available packs against the raw verb candidates to find the joint best.
/// If the user has a staged pack, that pack is strongly preferred.
///
/// Steps:
/// 1. Apply pack scoring with the active pack (if any).
/// 2. For unresolved cases (NoMatch / Ambiguous), try cross-pack scoring
///    by checking if a different pack's allowed set would disambiguate.
/// 3. Return the best joint result.
pub fn dual_decode(
    raw_candidates: &[VerbCandidate],
    context: &ContextStack,
    available_packs: &[super::context_stack::PackContext],
) -> DualDecodeResult {
    // First pass: score with the active pack (same as before).
    let mut candidates = raw_candidates.to_vec();
    apply_pack_scoring(&mut candidates, context);
    let primary_outcome = apply_ambiguity_policy(&candidates);

    // If we have a confident or proposed result, use it.
    match &primary_outcome {
        AmbiguityOutcome::Confident { verb, score } => {
            return DualDecodeResult {
                verb: verb.clone(),
                score: *score,
                pack_id: context.active_pack().map(|p| p.pack_id.clone()),
    ...
    // Second pass: try cross-pack scoring for ambiguous/no-match cases.
    // For each available pack, re-score the raw candidates as if that pack
    // were active. Pick the pack that produces the best confident/proposed result.
    let mut best_cross: Option<DualDecodeResult> = None;

    for pack in available_packs {
        // Skip the active pack (already tried).
        if context
            .active_pack()
            .map(|p| p.pack_id == pack.pack_id)
            .unwrap_or(false)
        {
            continue;
        }

        // Build a temporary context with this pack.
        let mut cross_candidates = raw_candidates.to_vec();
        apply_pack_scoring_with_pack(&mut cross_candidates, pack, context);
        let cross_outcome = apply_ambiguity_policy(&cross_candidates);

        let (verb, score, is_confident) = match &cross_outcome {
            AmbiguityOutcome::Confident { verb, score } => (verb.clone(), *score, true),
            AmbiguityOutcome::Proposed { verb, score } => (verb.clone(), *score, false),
            _ => continue,
        };

        let better = match &best_cross {
            None => true,
            Some(existing) => {
                // Prefer confident over proposed, then higher score.
                if is_confident && !matches!(existing.outcome, AmbiguityOutcome::Confident { .. }) {
                    true
                } else {
        ...
        if better {
            best_cross = Some(DualDecodeResult {
                verb,
                score,
                pack_id: Some(pack.pack_id.clone()),
                outcome: cross_outcome,
                scored_candidates: cross_candidates,
            });
        }
    }

    // If cross-pack found something, return it.
    if let Some(cross) = best_cross {
        return cross;
    }

    // No cross-pack improvement — return the primary result as-is.
    let (verb, score) = match &primary_outcome {
        AmbiguityOutcome::Ambiguous { top, .. } => (top.verb_fqn.clone(), top.score),
        AmbiguityOutcome::NoMatch => (String::new(), 0.0),
        AmbiguityOutcome::Confident { verb, score }
        | AmbiguityOutcome::Proposed { verb, score } => (verb.clone(), *score),
    };
...
fn apply_pack_scoring_with_pack(
    ...
    pack: &super::context_stack::PackContext,
    context: &ContextStack,
) {
    candidates.retain_mut(|c| {
        // Forbidden → remove entirely
        ...
        // Template step boost (only if context has a hint matching this pack)
        if context.is_template_step(&c.verb_fqn) {
            c.score += TEMPLATE_STEP_BOOST;
        }
...
pub fn recover_from_rejection(
    raw_candidates: &[VerbCandidate],
    context: &ContextStack,
    rejected_verb: &str,
) -> (Vec<VerbCandidate>, AmbiguityOutcome) {
    let mut candidates: Vec<VerbCandidate> = raw_candidates
        .iter()
        .filter(|c| c.verb_fqn != rejected_verb)
        .cloned()
        .collect();

    apply_pack_scoring(&mut candidates, context);
    let outcome = apply_ambiguity_policy(&candidates);
    (candidates, outcome)
}
...
mod tests {
    ...
    #[test]
    fn test_dual_decode_confident_with_active_pack() {
        let ctx = context_with_pack(
            vec!["kyc.create-case", "kyc.add-entity"],
            vec![],
            Some("kyc"),
        ...
        let result = dual_decode(&raw, &ctx, &packs);

        // Active pack boosts kyc.create-case above strong threshold
        assert_eq!(result.verb, "kyc.create-case");
        assert!(result.score >= STRONG_THRESHOLD);
        assert_eq!(result.pack_id, Some("test-pack".to_string()));
        assert!(matches!(result.outcome, AmbiguityOutcome::Confident { .. }));
    }

    #[test]
    fn test_dual_decode_cross_pack_disambiguation() {
        // No active pack — raw candidates are ambiguous
        let ctx = empty_context();
        let raw = vec![
            make_candidate("kyc.create-case", 0.62),
            make_candidate("cbu.create", 0.60),
        ];
        // Available packs: KYC pack allows kyc.create-case, book-setup allows cbu.create
        let packs = vec![
            make_pack(
                "kyc-case",
                vec!["kyc.create-case", "kyc.add-entity"],
        ...
        // Without a pack the two are ambiguous (margin < 0.05).
        // Cross-pack should find that kyc-case pack boosts kyc.create-case
        // to 0.62 + 0.10 (pack) + 0.03 (domain) = 0.75, making it confident.
        assert_eq!(result.verb, "kyc.create-case");
        assert!(result.score > 0.70);
        assert_eq!(result.pack_id, Some("kyc-case".to_string()));
    }

    #[test]
    fn test_dual_decode_no_packs_available() {
        let ctx = empty_context();
        let raw = vec![
            make_candidate("kyc.create-case", 0.62),
            make_candidate("cbu.create", 0.60),
        ];
        let packs: Vec<PackContext> = vec![];

        let result = dual_decode(&raw, &ctx, &packs);

        // No packs available — should be ambiguous
        assert!(matches!(result.outcome, AmbiguityOutcome::Ambiguous { .. }));
    }

    #[test]
    fn test_dual_decode_no_match_survives() {
        let ctx = empty_context();
        let raw = vec![make_candidate("cbu.create", 0.40)]; // below floor
        let packs = vec![make_pack(
            "kyc-case",
            vec!["kyc.create-case"],
            vec![],
        ...
        assert!(matches!(result.outcome, AmbiguityOutcome::NoMatch));
    }

    #[test]
    fn test_dual_decode_active_pack_preferred_over_cross() {
        // Active pack gives a proposed result; cross-pack also has a proposed result
        // but with lower score. Active pack should win.
        let ctx = context_with_pack(vec!["kyc.create-case"], vec![], Some("kyc"));
        let raw = vec![
            make_candidate("kyc.create-case", 0.60),
            make_candidate("cbu.create", 0.58),
        ];
        let packs = vec![make_pack(
            "book-setup",
            vec!["cbu.create"],
        ...
        // Active pack boosts kyc.create-case: 0.60 + 0.10 + 0.03 = 0.73 → Confident
        assert_eq!(result.verb, "kyc.create-case");
        assert_eq!(result.pack_id, Some("test-pack".to_string()));
    }

    // -- recover_from_rejection tests --

    #[test]
    fn test_recover_from_rejection_excludes_verb() {
        let ctx = empty_context();
        let raw = vec![
            make_candidate("kyc.create-case", 0.80),
            make_candidate("cbu.create", 0.75),
        ...
        let (candidates, outcome) = recover_from_rejection(&raw, &ctx, "kyc.create-case");

        // kyc.create-case should be excluded
        assert!(!candidates.iter().any(|c| c.verb_fqn == "kyc.create-case"));
        // cbu.create should now be top
        assert_eq!(candidates[0].verb_fqn, "cbu.create");
        assert!(matches!(
            outcome,
            AmbiguityOutcome::Confident { .. } | AmbiguityOutcome::Proposed { .. }
        ));
    }

    #[test]
    fn test_recover_from_rejection_no_match_when_all_excluded() {
        let ctx = empty_context();
        let raw = vec![make_candidate("kyc.create-case", 0.80)];

        let (candidates, outcome) = recover_from_rejection(&raw, &ctx, "kyc.create-case");

        assert!(candidates.is_empty());
        assert_eq!(outcome, AmbiguityOutcome::NoMatch);
    }

    #[test]
    fn test_recover_from_rejection_promotes_runner_up() {
        let ctx = context_with_pack(
            vec!["kyc.create-case", "kyc.add-entity"],
            vec![],
            Some("kyc"),
        ...
        let raw = vec![
            make_candidate("kyc.create-case", 0.75),
            make_candidate("kyc.add-entity", 0.65),
        ];
        ...
        assert_eq!(candidates.len(), 1);
        assert_eq!(candidates[0].verb_fqn, "kyc.add-entity");
        // 0.65 + 0.10 (pack) + 0.03 (domain) = 0.78 → Confident
        assert!(matches!(outcome, AmbiguityOutcome::Confident { .. }));
    }
}
