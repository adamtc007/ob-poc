...
mod db_eval {
    use super::*;
    use crate::agent::verb_surface::observe_state_reachability;
    use crate::mcp::verb_search::{soft_stage_flow, HybridVerbSearcher, VerbSearchResult};
    use std::path::Path;
    use std::sync::Arc;
    ...
    fn load_macro_scenario_fqns() -> HashSet<String> {
        ...
        if let Ok(reg) =
            crate::dsl_v2::load_macro_registry_from_dir(Path::new("config/verb_schemas/macros"))
        {
    ...
    async fn build_searcher(pool: &sqlx::PgPool) -> HybridVerbSearcher {
        ...
        let macro_index = {
            ...
            path.is_dir().then(|| {
                let reg =
                    crate::dsl_v2::load_macro_registry_from_dir(path).expect("macro registry");
    ...
    async fn phase1_single_trace_receipt() {
        ...
        let total_registry = crate::dsl_v2::execution::runtime_registry()
            .all_verbs()
            .count();
    ...
    async fn phase2_3_batch_eval() {
        ...
        // not state weakness.
        let verbs_with_state_preconditions = crate::dsl_v2::execution::runtime_registry()
            .all_verbs()
            .filter(|v| {
                v.lifecycle
                    .as_ref()
    ...
    async fn cbu_second_hit_metric() {
        use crate::mcp::verb_search::{check_ambiguity, VerbSearchOutcome};
