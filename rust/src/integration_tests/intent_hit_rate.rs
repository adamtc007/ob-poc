...
mod tests {
    ...
    async fn intent_hit_rate() {
        ...
        for (i, case) in tests.iter().enumerate() {
            ...
            if crate::agent::narration_engine::is_contextual_query(&case.utterance) {
            ...
            let result = match search_result {
                Ok(candidates) => {
                    ...
                    let ambiguity =
                        crate::mcp::verb_search::check_ambiguity(&candidates, threshold);
...
fn classify_outcome(
    ambiguity: &crate::mcp::verb_search::VerbSearchOutcome,
    candidates: &[crate::mcp::verb_search::VerbSearchResult],
    expected: &str,
    alts: &[String],
    _threshold: f32,
) -> (Outcome, String, Option<String>, Option<f32>) {
    use crate::mcp::verb_search::VerbSearchOutcome;
...
async fn build_test_searcher(pool: &PgPool) -> crate::mcp::verb_search::HybridVerbSearcher {
    ...
    use crate::dsl_v2::load_macro_registry_from_dir;
...
mod clarification_roundtrip {
    ...
    async fn test_clarification_phrases_resolve_to_their_verbs() {
        ...
        for case in &fixture.tests {
            ...
            let threshold = searcher.semantic_threshold();
            let ambiguity = crate::mcp::verb_search::check_ambiguity(&candidates, threshold);

            // Only test ambiguous cases — these are the "did you mean?" responses
            let is_ambiguous = matches!(
                ambiguity,
                crate::mcp::verb_search::VerbSearchOutcome::Ambiguous { .. }
                    | crate::mcp::verb_search::VerbSearchOutcome::Suggest { .. }
            );
            if !is_ambiguous {
                continue;
            }
