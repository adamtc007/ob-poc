...
use crate::repl::runbook::{EntryStatus, ExecutionMode, RunbookEvent};
...
fn user_entries(
    entries: &[crate::repl::runbook::RunbookEntry],
) -> Vec<&crate::repl::runbook::RunbookEntry> {
    entries.iter().filter(|e| !is_infra_verb(&e.verb)).collect()
}

/// Get the actual index of the Nth user entry in the full runbook
fn user_entry_index(entries: &[crate::repl::runbook::RunbookEntry], user_index: usize) -> usize {
    entries
        .iter()
        .enumerate()
