//! Consolidated DSL Manager - Unified Interface for All DSL Operations
//!
...
impl DslManager {
    ...
    /// Generate AST visualization for instance version
    pub async fn generate_instance_visualization(
        ...
        if let Some(ref ast_json) = version.ast_json {
            let visualization = self.build_ast_visualization_from_json(ast_json, options)?;
            Ok(visualization)
        } else {
            Err(DslError::CompilationError {
                message: "Version has not been compiled yet - no AST available".to_string(),
            })
        }
    }
    ...
    pub async fn generate_domain_enhanced_visualization(
        ...
        let version = self.get_version_by_id(version_id).await?;
        let _instance = self.get_instance(version.instance_id).await?;

        if let Some(ref ast_json) = version.ast_json {
            let base_visualization =
                self.build_ast_visualization_from_json(ast_json, &options.base_options)?;
    ...
    async fn get_cbu_info(&self, cbu_id: Uuid) -> DslResult<CbuInfo> {
        ...
        match row {
            Some(row) => Ok(CbuInfo {
                ...
                nature_purpose: row.nature_purpose,
    ...
    async fn store_dsl_source_with_ob_index(
        ...
        let dsl_ob_version_id = sqlx::query_scalar!(
            r#"INSERT INTO "ob-poc".dsl_ob (version_id, cbu_id, dsl_text, created_at)
               VALUES ($1, $2, $3, NOW())
               RETURNING version_id"#,
            version_id,
            cbu_id.to_string(), // Note: current schema uses VARCHAR for cbu_id
            dsl_content
        )
        .fetch_one(self.domain_repository.pool())
        .await
...
mod tests {
    ...
    /// Create test DSL manager with real database connection
    ...
    async fn create_test_cbu(pool: &PgPool) -> Uuid {
        ...
        sqlx::query!(
            r#"INSERT INTO "ob-poc".cbus (cbu_id, name, description, nature_purpose)
               VALUES ($1, $2, $3, $4)"#,
            cbu_id,
            "Test CBU for DSL Manager",
            "Test CBU Description",
            "Testing DSL Manager functionality"
        )
        .execute(pool)
        .await
    ...
    async fn test_create_onboarding_request_with_database() {
        ...
        match result {
            Ok(creation_result) => {
                ...
                let dsl_content = stored_dsl.unwrap().dsl_text;
                assert!(
        ...
        sqlx::query!(
            ...
            test_cbu_id.to_string()
        )
        .execute(pool)
        .await
        .expect("Failed to clean up DSL records");

        sqlx::query!(r#"DELETE FROM "ob-poc".cbus WHERE cbu_id = $1"#, test_cbu_id)
            .execute(pool)
            .await
