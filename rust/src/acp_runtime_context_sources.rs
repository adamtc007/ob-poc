//! Session-derived source collection for Slice 2 ACP runtime context.
//!
//! This module is transport-adjacent: it reads already scoped session state and
//! produces an `AcpRuntimeContextSource`. Redaction and deterministic hashing
//! remain owned by `acp_runtime_context`.

use sha2::{Digest, Sha256};
use std::collections::BTreeMap;
use uuid::Uuid;

use crate::acp_runtime_context::AcpRuntimeContextSource;
...
///
/// ```rust,ignore
/// let source = build_session_runtime_context_source(input);
/// assert_eq!(source.pack_id, "onboarding-request");
/// ```
...
mod tests {
    use super::*;
    use crate::acp_runtime_context::build_acp_runtime_context_projection;
    ...
    #[test]
    fn fixture_backed_real_source_subset_uses_session_refs_without_forbidden_text() {
        let fixture_ids = ["S2-ONB-008", "S2-CBU-004", "S2-SRDEF-003"];
        for fixture_id in fixture_ids {
            let fixture = slice2_fixture(fixture_id);
            let session = session_for_fixture(&fixture);
            let source = build_session_runtime_context_source(AcpRuntimeContextBuildInput {
                pack_id: fixture.expected_pack.clone(),
                selected_ref: fixture.expected_verb.clone(),
                static_envelope_hash: format!("static-envelope-{}", fixture.expected_pack),
                session: Some(&session),
                missing_required_args: Vec::new(),
            });
            let projection = build_acp_runtime_context_projection(source);
            ...
            assert!(projection.verified, "{} verified", fixture.id);
            assert_eq!(
            ...
            assert!(
                projection.snapshot_id.starts_with("repl-session:"),
                "{} snapshot id",
                fixture.id
            );
            assert!(
                projection
                    .source_refs
                    .iter()
                    .any(|source_ref| source_ref == &format!("repl_session:{}", session.id)),
                "{} source refs should cite session id",
                fixture.id
            );
            assert!(
                projection
                    .source_refs
                    .iter()
                    .any(|source_ref| source_ref.starts_with("repl_session:last_active_at:")),
                "{} source refs should cite snapshot timestamp",
                fixture.id
            );
            for expected_field in expected_real_source_fields(&fixture) {
                assert!(
            ...
            for forbidden_field in &fixture.forbidden_runtime_fields {
                ...
                assert!(
                    ...
                    "{} leaked forbidden value for `{}`",
                    fixture.id,
                    forbidden_field
                );
            }
            assert!(!serialized.contains("Confidential"));
            assert!(!serialized.contains("Raw free text"));
        }
    }

    fn slice2_fixture(id: &str) -> Slice2Fixture {
        SLICE_2_FIXTURES_JSONL
            .lines()
            .filter(|line| !line.trim().is_empty())
            .map(|line| serde_json::from_str::<Slice2Fixture>(line).expect("fixture parses"))
            .find(|fixture| fixture.id == id)
            .unwrap_or_else(|| panic!("fixture `{id}` should exist"))
    }

    fn expected_real_source_fields(fixture: &Slice2Fixture) -> Vec<&str> {
        fixture
            .expected_runtime_fields
            .iter()
            .filter_map(|field| match field.as_str() {
                "workbook_step_statuses" => Some("workbook_step_statuses"),
                "run_sheet_cursor" => Some("run_sheet_cursor"),
                "fsm_state" => Some("fsm_state"),
                "cbu_id" => Some("cbu_id"),
                "product_binding_ids" => Some("product_binding_ids"),
                "binding_status" => Some("binding_status"),
                "missing_resource_codes" => Some("missing_resource_codes"),
                "missing_resource_count" => Some("missing_resource_count"),
                "operation_required_count" => Some("operation_required_count"),
                _ => None,
            })
            .collect()
    }

    fn session_for_fixture(fixture: &Slice2Fixture) -> ReplSessionV2 {
        match fixture.id.as_str() {
            "S2-ONB-008" => onboarding_fixture_session(),
            "S2-CBU-004" => cbu_fixture_session(),
            "S2-SRDEF-003" => taxonomy_fixture_session(),
            other => panic!("unsupported fixture `{other}`"),
        }
    }

    fn onboarding_fixture_session() -> ReplSessionV2 {
        let mut session = ReplSessionV2::new();
        let cbu_id = Uuid::new_v4();
        session.cbu_ids.push(cbu_id);
        session.runbook_plan_cursor = Some(2);
        session.runbook_plan = Some(RunbookPlan::new(
            session.id,
            vec![
                plan_step(
                    1,
                    WorkspaceKind::OnBoarding,
                    cbu_id,
                    "onboarding.resolve-request",
                ),
                plan_step(
                    2,
                    WorkspaceKind::OnBoarding,
    ...
    fn cbu_fixture_session() -> ReplSessionV2 {
        let mut session = ReplSessionV2::new();
        let cbu_id = Uuid::new_v4();
        let product_id = Uuid::new_v4();
        session.cbu_ids.push(cbu_id);
        session.bindings.insert(
            "product_binding".to_string(),
            serde_json::json!({
                "id": product_id.to_string(),
                "product_name": "Confidential Product Name",
                "commercial_terms": "Raw free text commercial terms"
            }),
        );
        session
    }

    fn taxonomy_fixture_session() -> ReplSessionV2 {
        let mut session = ReplSessionV2::new();
        session.bindings.insert(
            "missing_resource_codes".to_string(),
            serde_json::json!({
                "codes": ["SRDEF:MISSING:1", "SRDEF:MISSING:2"],
                "resource_name": "Confidential Resource Name",
                "free_text": "Raw free text discovery payload"
            }),
        );
        session.bindings.insert(
            "srdef_discovery_ids".to_string(),
    ...
    fn plan_step(
        ...
        RunbookPlanStep {
            seq,
            workspace,
            constellation_map: "fixture".to_string(),
            subject_kind: SubjectKind::Handoff,
            subject_binding: EntityBinding::Literal { id: subject_id },
            verb: VerbRef {
                verb_fqn: verb_fqn.to_string(),
                display_name: "Fixture Verb".to_string(),
            },
            sentence: "Confidential sentence text must not leak".to_string(),
            args: BTreeMap::new(),
            preconditions: Vec::new(),
            expected_effect: "fixture effect".to_string(),
            depends_on: Vec::new(),
            status: PlanStepStatus::Ready,
        }
    }
