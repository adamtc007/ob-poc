...
//!
//! Handles import and lifecycle transitions of trading profile documents.
//! The trading profile is a single JSONB document that is the source of truth
//! for a CBU's trading configuration.
//!
//! `trading-profile.approve` (PENDING_REVIEW -> ACTIVE) inlines a
//! materialization step that syncs the document to operational tables:
...
//!
//! `trading-profile.materialize` (a standalone verb parsing the document as
//! the incompatible flat `TradingProfileDocument` shape, which no real
//! ACTIVE/tree-shaped document ever satisfies) was removed
//! 2026-08-20 (EOP-PLAN-MANDATE-FIX-001 F3, B2) — zero rows were ever
//! written to its audit table, `trading_profile_materializations` (also
//! dropped). The shared `materialize_*` helpers below remain because
//! `trading-profile.approve` still calls them directly.
//!
...
use ob_poc_trading_profile::{
    ...
    InvestmentManagerMandate, IsdaAgreementConfig, StandingInstruction,
