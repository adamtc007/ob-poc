...
mod tests {
    ...
    #[tokio::test]
    async fn test_detect_passport() {
        let result =
            DocumentTypeDetector::detect_type("application/pdf", "john_doe_passport.pdf", &[])
                .await;

        assert_eq!(result, Some("PASSPORT".to_string()));
    }

    #[tokio::test]
    async fn test_detect_bank_statement() {
        let result =
            DocumentTypeDetector::detect_type("application/pdf", "bank_statement_2024.pdf", &[])
                .await;

        assert_eq!(result, Some("BANK_STATEMENT".to_string()));
    }

    #[tokio::test]
    async fn test_detect_utility_bill() {
        let result = DocumentTypeDetector::detect_type(
            "application/pdf",
            "utility_bill_march_2024.pdf",
            &[],
        )
        .await;

        assert_eq!(result, Some("UTILITY_BILL".to_string()));
    }

    #[tokio::test]
    async fn test_detect_drivers_license() {
        let result =
            DocumentTypeDetector::detect_type("image/jpeg", "drivers_license_scan.jpg", &[]).await;

        assert_eq!(result, Some("DRIVERS_LICENSE".to_string()));
    }

    #[tokio::test]
    async fn test_detect_generic_pdf() {
        let result =
            DocumentTypeDetector::detect_type("application/pdf", "random_document.pdf", &[]).await;

        assert_eq!(result, Some("GENERIC_PDF".to_string()));
    }

    #[tokio::test]
    async fn test_detect_with_confidence() {
        let result = DocumentTypeDetector::detect_type_with_confidence(
            "application/pdf",
        ...
        let (doc_type, confidence) = result.unwrap();
        assert_eq!(doc_type, "PASSPORT");
