...
mod tests {
    ...
    fn test_confidence_calculation() {
        ...
        let bad_dsl = "Here's a DSL statement that creates...";
    ...
    fn test_cost_estimation() {
        ...
        let cost = service.estimate_openai_cost("gpt-3.5-turbo", 1000);
        assert_eq!(cost, 0.002);

        let cost = service.estimate_gemini_cost(1000);
        assert_eq!(cost, 0.00025);
    }

    fn create_test_service() -> RealAiEntityService {
        let rag_system = CrudRagSystem::new();
        let prompt_builder = CrudPromptBuilder::new();
        let config = AiServiceConfig::default();

        RealAiEntityService::new(None, None, config, rag_system, prompt_builder)
            .unwrap_or_else(|_| panic!("Failed to create test service"))
    }
}
