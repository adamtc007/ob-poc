...
mod tests {
    ...
    async fn test_mock_integration() {
        let integration = MockIntegration::new("test_mock");

        let mut params = HashMap::new();
        params.insert("from".to_string(), serde_json::json!("risk-engine"));
        ...
        let result = integration.execute(&operation, &context).await.unwrap();
        assert!(result.get("risk_score").is_some());
    }
    ...
    async fn test_integration_registry() {
        ...
        let integration = registry.get("test");
