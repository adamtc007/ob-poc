...
mod tests {
    use super::*;
    use crate::dsl::operations::ExecutableDslOperation as DslOperation;
    use crate::execution::{ExecutionContext, StateMetadata};
    use std::collections::HashMap;
    ...
    #[tokio::test]
    async fn test_ownership_limits_rule() {
        let rule = OwnershipLimitsRule::new();
        let context = create_test_context();
        let state = create_test_state();

        let mut params = HashMap::new();
        params.insert("from".to_string(), serde_json::json!("entity-a"));
        params.insert("to".to_string(), serde_json::json!("entity-b"));
        params.insert("type".to_string(), serde_json::json!("HAS_OWNERSHIP"));
        params.insert(
            "properties".to_string(),
            serde_json::json!({ "percent": 75.0 }),
        );
        ...
        let result = rule.validate(&operation, &state, &context).await.unwrap();
        assert!(result.valid);
    }

    #[tokio::test]
    async fn test_ownership_limits_rule_violation() {
        let rule = OwnershipLimitsRule::new();
        let context = create_test_context();
        let state = create_test_state();
    ...
    #[tokio::test]
    async fn test_document_evidence_rule() {
        let rule = DocumentEvidenceRule::new();
        let context = create_test_context();
        let state = create_test_state();
        ...
        params.insert("type".to_string(), serde_json::json!("HAS_OWNERSHIP"));
        params.insert(
            "evidenced-by".to_string(),
            serde_json::json!(["doc-certificate-123"]),
        );
    ...
    async fn test_business_rule_registry() {
        ...
        let results = registry
            .validate_operation(&operation, &state, &context)
            .await
            .unwrap();
