//! Default generation for onboarding pipeline steps.
//!
...
use crate::sem_reg::membership::{MembershipKind, MembershipRuleBody};
...
fn crud_contract(
    ...
    // Build args: create/update take the entity's required attributes as args
...
pub fn default_taxonomy_fqns_for_entity_type(entity_type: &EntityTypeDefBody) -> Vec<String> {
    ...
    // Always place in the entity classification taxonomy
...
pub fn membership_rule_for_entity_in_taxonomy(
    ...
    // Derive the FQN as: taxonomy_fqn + "." + entity_type_fqn
    let rule_fqn = format!("{taxonomy_fqn}.{entity_type_fqn}");

    // Derive the node FQN: use the domain portion of the entity type FQN
    // to find the appropriate node in the taxonomy.
...
mod tests {
    ...
    fn test_default_attributes_from_entity_type() {
        ...
        assert_eq!(attrs.len(), 3); // 2 required + 1 optional
        ...
        assert!(attrs[2].constraints.is_none()); // optional → no constraints
    }
    ...
    fn test_default_verb_contracts() {
        ...
        // create should produce the entity type
        let create = contracts.iter().find(|c| c.action == "create").unwrap();
        assert!(create.produces.is_some());
        assert_eq!(
            create.produces.as_ref().unwrap().entity_type,
            "entity.test-widget"
        );
        assert!(create.produces_focus);
        assert!(!create.requires_subject);

        // get should require scope
        let get = contracts.iter().find(|c| c.action == "get").unwrap();
        assert!(!get.preconditions.is_empty());
        assert!(get.requires_subject);
    }
    ...
    fn test_columns_for_entity_in_view() {
        ...
        assert_eq!(cols.len(), 2); // only required attributes
    ...
    #[test]
    fn test_crud_contract_args() {
        let et = sample_entity_type();
        let contracts = default_verb_contracts_for_entity_type(&et);

        // create should have args for required attributes
        let create = contracts.iter().find(|c| c.action == "create").unwrap();
        assert_eq!(create.args.len(), 2); // widget-name, widget-status
        assert!(create.args.iter().all(|a| a.required));

        // update should have id + required attrs (non-required)
        let update = contracts.iter().find(|c| c.action == "update").unwrap();
        assert_eq!(update.args.len(), 3); // id + widget-name + widget-status
        assert!(update.args[0].required); // id is required
        assert!(!update.args[1].required); // attrs not required on update

        // delete has just id
        let delete = contracts.iter().find(|c| c.action == "delete").unwrap();
        assert_eq!(delete.args.len(), 1);
        assert_eq!(delete.args[0].name, "id");
    }
}
