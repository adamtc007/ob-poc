...
mod tests {
    ...
    fn test_migration_manager_creation() {
        let manager = VocabularyMigrationManager::new();
        assert!(manager.migration_rules.is_empty());
        assert!(manager.version_mappings.is_empty());
    }

    #[test]
    fn test_add_migration_rule() {
        let mut manager = VocabularyMigrationManager::new();
        let rule = MigrationRule {
            rule_id: Uuid::new_v4(),
            rule_name: "test_rename".to_string(),
            from_version: "1.0.0".to_string(),
            to_version: "2.0.0".to_string(),
            transformation: VocabularyTransformation::Rename {
                old_verb: "old.action".to_string(),
                new_verb: "new.action".to_string(),
            },
            rollback_available: true,
        };

        manager.add_migration_rule(rule);
        assert_eq!(manager.migration_rules.len(), 1);
        assert!(manager.version_mappings.contains_key("1.0.0"));
    }

    #[tokio::test]
    async fn test_apply_rename_transformation() {
        let manager = VocabularyMigrationManager::new();
        let mut vocabularies = vec![create_test_vocabulary("test", "old.action")];

        let result = manager
            .apply_rename("old.action", "new.action", &mut vocabularies)
            .await;

        assert!(result.is_ok());
        assert_eq!(vocabularies[0].verb, "new.action");
    }

    #[tokio::test]
    async fn test_apply_deprecation_transformation() {
        let manager = VocabularyMigrationManager::new();
        let mut vocabularies = vec![create_test_vocabulary("test", "deprecated.action")];
    ...
    fn test_migration_plan_creation() {
        ...
        manager.add_migration_rule(rule);
        let plan = manager.create_migration_plan("1.0.0", "2.0.0").unwrap();

        assert_eq!(plan.from_version, "1.0.0");
        assert_eq!(plan.to_version, "2.0.0");
        assert_eq!(plan.transformations.len(), 1);
        assert_eq!(plan.estimated_impact.breaking_changes, 1);
    }
}
