//! ob-poc verb compiler — consumer-side verb dispatch for ob-poc DSL programs.
//!
//! This is the migration target for `dsl_core::compiler::compile_ob_poc_verb`.
//! Once `dsl_analysis::planning_facade` is migrated to pass an explicit handler,
//! the duplicate in dsl-core will be removed and this becomes the sole home.
//!
//! # Adding new ob-poc verbs
//!
//! Add a new arm to `ob_poc_verb_handler`. Return `Some(Ok(...))` on success,
//! `Some(Err(msg))` on argument error, `None` to fall through (not ob-poc).
//! Do NOT add new verbs to `dsl_core::compiler::compile_ob_poc_verb`.
...
pub fn ob_poc_verb_handler(
    ...
    match full_verb.as_str() {
        ...
        // Not an ob-poc verb — fall through to dsl-core's built-in dispatch.
        _ => None,
    }
}
