-- Normalize SemOS AttributeDef.data_type before projecting to attribute_registry.value_type.
--
-- AttributeDataType::Enum serializes as a tagged JSON object, while the
-- operational registry stores the compact check value `enum`.  This trigger is
-- the SemOS -> operational projection boundary, so it must accept canonical
-- SemOS JSON and write the legacy value_type vocabulary.
