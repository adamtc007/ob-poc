...
impl AiDslGenerator {
    ...
    async fn log_generation(&self, instruction: &str, dsl: &str, ast: &CrudStatement) -> Result<Uuid> {
        let id = Uuid::new_v4();
...
impl CrudExecutor {
    ...
    async fn create_cbu(&self, create: &CreateCbu) -> Result<ExecutionResult> {
        ...
        sqlx::query!(
            ...
            format!("{:?}", create),
            format!("CREATE CBU WITH nature_purpose \"{}\" AND source_of_funds \"{}\"",
    ...
    async fn connect_entity(&self, connect: &ConnectEntity) -> Result<ExecutionResult> {
        ...
        Ok(ExecutionResult {
            ...
            message: format!("Connected entity {} to CBU {} with role {}", 
...
mod tests {
    ...
    async fn test_create_cbu_flow() -> Result<()> {
        ...
        let result = service.process_instruction(
        ...
        let read_result = service.process_instruction(
            &format!("Read CBU {}", cbu_id)
        ).await?;
...
pub async fn run_demo(pool: PgPool) -> Result<()> {
    ...
    let instruction3 = format!("Connect entity {} to CBU {} as role {}", 
